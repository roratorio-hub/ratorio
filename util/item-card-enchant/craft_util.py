...
def loadCardDict() -> dict:
    ...
    pattern = r'\[(\d+),(\d+),"([^,]+)",[^,]*,[^,]*,.*0\]'
    with open(f'{script_dir}/../../engine/equip/card.dat.js', 'r', encoding='utf-8') as file:
        js_code = file.read()
...
def loadItemList():
    ...
    with open(f'{script_dir}/../../engine/equip/item.dat.js', 'r', encoding='utf-8') as file:
        js_code = file.read()
    matches = re.findall(pattern, js_code)
...
def loadSlotInfoList():
    ...
    with open(f'{script_dir}/../../engine/data/mig.enchlist.dat.js', 'r', encoding='utf-8') as file:
        js_code = file.read()
    matches = re.findall(pattern, js_code)
...
def getLatestIdFromItemSet():
    pattern = r'w_SE\[(\d+)] = \[[^;]+;'
    with open(f'{script_dir}/../../engine/equip/itemset.dat.js', 'r', encoding='utf-8') as file:
        js_code = file.read()
    matches = re.findall(pattern, js_code)
...
def getLatestMonsterId():
    pattern = r'\[(\d+),.+'
    with open(f'{script_dir}/../../engine/monster/monster.dat.js', 'r', encoding='utf-8') as file:
        js_code = file.read()
    matches = re.findall(pattern, js_code)
    return max([int(id) for id in matches])
...
def getTimeItemRecordArray(item_id):
    ...
    with open(f'{script_dir}/../../engine/equip/timeitem.dat.js', 'r', encoding='utf-8') as file:
        js_code = file.read()
    matches = re.findall(pattern, js_code)
    return matches
