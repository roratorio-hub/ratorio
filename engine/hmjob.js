...
import { changeJobSettings } from "./equip.js";
...
import { LEARNED_SKILL_MAX_COUNT, LearnedSkillSearch, OnClickSkillSWLearned, n_A_LearnedSkill } from "./learnedskill.js";
import { MOB_CONF_DEBUF_ID_JACK_FROST_NOVA, MOB_CONF_DEBUF_ID_TOXIN_OF_MANDARA, n_B_IJYOU } from "./mobconfdebuf.js";
import {
...
export function CalcStatusPoint(bIgnoreAutoCalc, bIgnorePointCap = false) {
	...
	// ベースレベルを取得する
	var blvSelected = eval(_cf.A_BaseLV.value);

	// ステータス値を取得する
	var stValSTR = eval(_cf.A_STR.value);
	var stValAGI = eval(_cf.A_AGI.value);
	var stValVIT = eval(_cf.A_VIT.value);
	var stValINT = eval(_cf.A_INT.value);
	var stValDEX = eval(_cf.A_DEX.value);
	var stValLUK = eval(_cf.A_LUK.value);

	// 特性ステータス値を取得する
	var stValPOW = eval(_cf.A_POW.value);
	var stValSTA = eval(_cf.A_STA.value);
	var stValWIS = eval(_cf.A_WIS.value);
	var stValSPL = eval(_cf.A_SPL.value);
	...
	if (bPointCap && !bIgnorePointCap) {
		...
		if (((stPointEarned - stPointUsed) < 0) || ((stTSPointEarned - stTSPointUsed) < 0)) {
			_cf.A_STR.value = g_STR;
				_cf.A_AGI.value = g_AGI;
				_cf.A_VIT.value = g_VIT;
				_cf.A_INT.value = g_INT;
				_cf.A_DEX.value = g_DEX;
				_cf.A_LUK.value = g_LUK;
				_cf.A_POW.value = g_POW;
				_cf.A_STA.value = g_STA;
				_cf.A_WIS.value = g_WIS;
...
import { MONSTER_DATA_INDEX_MRES, MONSTER_DATA_INDEX_RES } from "./const/EnumMonsterDataIndex.js";
