 ...
 *
 * `createBattleScratchTemplate()`（残件台帳 B-09 Phase 4）: `ComputeBattleResult()` の
 * 入口で `Object.assign(CS, createBattleScratchTemplate())` を呼ぶことで、700箇所超の
 * 複合代入コードには一切触れずに「呼び出しごとに初期値へ戻る」性質を得る。
 * 配列/オブジェクト値は呼び出しのたびに新しいインスタンスを返す（参照の使い回しを避けるため）。
 */

/** @returns {object} CS の初期値のテンプレート（新規インスタンス） */
export function createBattleScratchTemplate() {
    return {
...
export const CS = createBattleScratchTemplate();
