...
1. アイテム情報を定義する
    変更箇所: item.dat.js
        ItemObjNew に新しいアイテムデータを追加する

2. エンチャント効果を定義する
    変更箇所: card.dat.js

        あらかじめ CardObjNew から対象のエンチャントIDをすべて探し出してメモしておく
        もし新規追加されたエンチャントで既存のIDが存在しない場合は CardObjNew に新しいエントリを追加すること
        例えば cardId = 2791 として潜在覚醒(流星落下I)を追加する場合は以下の通り
        [2791,99,"潜在覚醒(流星落下I)","",100000000005959,1,0],

    変更箇所: mig.enchlist.dat.js

        メモしておいた cardId を使って sourceArray に新しいエンチャントリストを追加する

        さらに reverseResolveArrayItemId にエンチャントリストの主キーを追加する
        reverseResolveArrayItemId は itemId を引数にとり、該当する配列インデックスにあるエンチャント情報の主キーを返すために使われる
        例えば itemId = 4994 に sourceArray[621] のエンチャントリストを関連付ける場合は以下のようにする
        g_constDataManager.enchListDataManager.reverseResolveArrayItemId[4994] = [621];

        ただし、現在は上記のように reverseResolveArrayItemId のインデックスを指定して挿入するのではなく
        reverseResolveArrayItemId の初期化時に 4994 番目の要素を探し出して [621] をセットする仕様になっている
        わかりにくいので今後修正する予定

3. セット効果を定義する
    変更箇所: itemset.dat.js

        セット効果がある場合は w_SE に新しいリレーションを追加する
        このとき itemId と cardId を区別するために、itemId は正の数で指定し、cardId は負の数で指定する
        例えば itemId = 4995 に cardId = 2417 とのセット効果である itemId = 4996 を適用する場合は以下のようにする
        [4996,4995,-2417],

        このとき、もし初めて登場したセット効果ならば item.dat.js にセット効果を追加しておく必要がある
        この場合は itemId = 4996 をあらかじめ登録してある

        さらに ItemIdToSetIdMap にリレーションの主キーを追加する
        ItemIdToSetIdMap は itemId を引数にとり、該当する配列インデックスにあるセット効果の主キーを返すために使われる
        例えば itemId = 4995 に w_SE[2391] のセット効果を適用したい場合は、ItemIdToSetIdMap[4995] = [2391] をセットする必要がある
        
        セット効果のインデックスを簡単に確認する方法は今のところ無い
        先ほどの例だと [4996,4995,-2417], が w_SE 配列の何番目にセットされたのか数えるしか無い
        わかりにくいので今後修正する予定
