(function () {
	...
	ItemObjNew = [
		...
		[4300,3,146,280,2474,1,340,185,"ガーディアンナイツクレイモア","カアテイアンナイツクレイモア","精錬値が1上がる度に追加で、一部のダメージを反射する効果で受けるダメージ - 10%(計算機未対応)",88,230,194,1,10000007005445,1,9019445,100,10010000000005440n,1,10000000000000077n,40,10000000010000017n,250,10000000010000068n,25,0],
		...
		[4306,11,147,150,2474,1,280,185,"ロイヤルカタール","ロイヤルカタアル","精錬値が1上がる度に追加で、一部のダメージを反射する効果で受けるダメージ - 10%(計算機未対応)",88,250,194,1,10000007005474,1,9019474,200,10000000000000077n,40,10020000000005474n,1,10000000010000017n,250,10000000010000068n,25,0],
		...
		[4310,10,149,160,2474,1,90,185,"ガーディアンナイツアーチャーボウ","カアテイアンナイツアアチヤアホウ","精錬値が1上がる度に追加で、一部のダメージを反射する効果で受けるダメージ - 10%(計算機未対応)<BR>精錬値が9以上の時、[トラップ研究]の習得Lvが1上がる度に追加で、Int + 3 , Dex + 3",199,500,88,170,194,1,10000007005505,4,10000000000000077n,40,10100000000005498n,1,10100000000005497n,1,10000000010000017n,250,10000000010000068n,25,0],
		...
		[4431,20,0,180,3884,1,300,100,"アンティークガトリングガン","アンテイイクカトリンクカン","純粋なAgiが10上がる度に追加で「ラウンドトリップ」で与えるダメージ+7%。<BR>純粋なAgiが110以上の時、追加で「ラウンドトリップ」で与えるダメージ+30%。",88,120,10000000005833,5,119836,8000,10000000000000077n,40,10020000000005828n,1,10020000000005835n,1,10000000010000017n,250,10000000010000068n,25,0],
		...
		[4485,50,0,10,3770,1,50,100,"フェイスオブイグドラシル","フエイスオフイクトラシル","※全ての基本ステータス+3は、集中力向上等の計算に含まれます。（2022/05/25実測）",19,10,7,3,79,20,77,20,27,10,28,10,29,10,97,10,98,10,99,10,7000027,15,7000028,15,7000029,15,7000097,15,7000098,15,7000099,15,7000074,20,9000027,15,9000028,15,9000029,15,9000097,15,9000098,15,9000099,15,9000243,13,0],
		...
		[4654,10,153,160,4,1,90,100,"トライアングルディザスター","トライアンクルテイサスタア","超越段階が1以上の時、[トライアングルショット]使用時、一定確率で20秒間、[アンリミット]Lv5状態になる<br>装備解除時、[アンリミット]を解除する",88,170,10000000005608,1,100212,3,100215,3,10000000000000077n,40,10000000010000017n,250,10000000010000068n,25,0],
	...
	ItemObjNew[5199] = [5199,50,160,10,0,1,50,200,"時間のサークレット(シャドウクロス)","シカンノサアクレツトシヤトウクロス","",194,1,79,20,77,20,243,13,100262,4,100269,4,7000015,10,7000016,10,7019461,2000,7019797,60000,9000217,10,100000009006013,1,100000009006021,1,100000009006016,1,100000009006020,1,0];
	...
	// --- 2024/06/18 ラグ缶 ---
	ItemObjNew[5226] = [5226,61,0,60,0,0,30,100,"グラキエースアラネア","クラキエエスアラネア","精錬値が5以上の時、追加で移動速度増加<BR>精錬値が7以上の時、固定詠唱時間-70%",194,1,77,40,243,35,100015,3,100016,3,0];
	ItemObjNew[5227] = [5227,60,177,100,0,1,100,200,"ふかふかフォーンローブ","フカフカフオオンロオフ","精錬値が7以上の時、絶対にノックバックしない",243,7,100000000006247,1,100000000006248,1,100265,7,7000012,10,9000194,1,9019923,2500,9019928,2500,0];
	...
	ItemObjNew[5229] = [5229,61,0,95,0,0,120,100,"プロンテラ軍の盾","フロンテラクンノタテ","精錬値が7以上の時、追加で一部の回復アイテムによるHP回復量 + 15%",15,15,16,15,243,35,5000015,15,5000016,15,7000092,15,0];	
	// --- 2024/07/16 ラグ缶 ---
	...
	ItemObjNew[5231] = [5231,60,162,100,0,1,100,200,"リュカントロポス","リユカントロホス","",243,7,100000000006051,1,100000000006047,1,100260,7,7000012,10,7000077,5,7000079,5,9000194,1,9019498,200,0];
	ItemObjNew[5232] = [5232,61,152,170,0,0,400,100,"メサルティム","メサルティム","",194,1,19,15,79,40,77,40,100015,3,100016,3,5000068,25,7000065,15,0];	
	// --- 2024/08/06 ブートキャンプ ---
	...
	ItemObjNew[5235] = [5235,60,163,100,0,1,100,200,"スペキュレーション","スヘキユレエシヨン","",243,7,100000000006069,1,100000000006070,1,100000000006068,1,100000000006067,1,100265,7,7000012,10,7000077,5,7000079,5,9000194,1,9011527,100,0];
	ItemObjNew[5236] = [5236,60,161,100,0,1,100,200,"着ぐるみペタル(アルビノ)","キクルミヘタルアルヒノ","",243,7,100000000006037,1,100000000006031,1,100260,7,7000012,10,7000077,5,7000079,5,9000194,1,10000009005741,2,10000009005742,2,0];
	// --- 2024/08/28 ---
	...
	// --- 2024/09/17 ---
	ItemObjNew[5278] = [5278,60,173,100,0,1,100,200,"ブーゲンビリア","フウケンヒリア","",243,7,100000000006197,1,100000000006198,1,100000000006199,1,100000000006200,1,100000000006201,1,100000000006202,1,100265,7,7000012,10,7000079,5,7000077,5,9000194,1,9019977,500,0];
	...
	ItemObjNew[5323] = [5323,60,160,100,0,1,100,200,"ヘイトレッドダークネス","ヘイトレツトタアクネス","",243,7,100000000006013,1,100000000006021,1,100000000006016,1,100000000006020,1,100260,7,7000012,10,7000079,5,7000077,5,9000194,1,30000009005469,1,0];
	ItemObjNew[5324] = [5324,60,170,100,0,1,100,200,"ティルナノーグ","テイルナノオク","",243,7,100000000006149,1,100000000006146,1,100000000006147,1,100000000006150,1,100000000006148,1,100000000006157,1,100265,7,7000012,10,7000079,5,7000077,5,9000194,1,9011662,100,0];
	// --- 2024/10/30 ---
	...
	// --- 2024/11/12 ---
	ItemObjNew[5363] = [5363,60,167,100,0,1,100,200,"レディアントピュリファイ","レテイアントヒユリフアイ","",243,7,100000000006113,1,100000000006123,1,100000000006124,1,100260,7,7000012,10,7000079,5,7000077,5,9000194,1,11622,100,0];
	ItemObjNew[5364] = [5364,60,164,100,0,1,100,200,"ワールドオール","ワアルトオオル","精錬値が9以上の時、追加でスキル使用時、[魔導ギア燃料]を消費しない",243,7,100000000006237,1,100000000006238,1,100260,7,7000012,10,7000079,5,7000077,5,9000194,1,0];
	...
	ItemObjNew[5378] = [5378,72,192,0,1900000,0,0,200,"シーズナルウェポン","シイスナルウエホン","精錬値が1上がる度に追加でSP自然回復量 + 2%<br>精錬値が7以上の時、追加でSP自然回復量 + 10%<br>精錬値が9以上の時、追加でSP自然回復量 + 20%",194,1,0];
	ItemObjNew[5379] = [5379,73,192,0,2000000,0,0,200,"シーズナルシールド","シイスナルシイルト","精錬値が1上がる度に追加でSP自然回復量 + 1%<br>精錬値が7以上の時、追加でSP自然回復量 + 5%<br>精錬値が9以上の時、追加でSP自然回復量 + 10%",194,1,0];
	ItemObjNew[5380] = [5380,77,192,0,1800000,0,0,200,"シーズナルアーマー","シイスナルアアマア","精錬値が1上がる度に追加でSP自然回復量 + 1%<br>精錬値が7以上の時、追加でSP自然回復量 + 5%<br>精錬値が9以上の時、追加でSP自然回復量 + 10%",194,1,0];
	ItemObjNew[5381] = [5381,80,192,0,2100000,0,0,200,"シーズナルシューズ","シイスナルシユウス","精錬値が1上がる度に追加でSP自然回復量 + 1%<br>精錬値が7以上の時、追加でSP自然回復量 + 5%<br>精錬値が9以上の時、追加でSP自然回復量 + 10%",194,1,0];
	ItemObjNew[5382] = [5382,82,192,0,2200000,0,0,200,"シーズナルイヤリング","シイスナルイヤリンク","精錬値が1上がる度に追加でSP自然回復量 + 1%<br>精錬値が7以上の時、追加でSP自然回復量 + 5%<br>精錬値が9以上の時、追加でSP自然回復量 + 10%",194,1,0];
	ItemObjNew[5383] = [5383,83,192,0,2300000,0,0,200,"シーズナルペンダント","シイスナルヘンタント","精錬値が1上がる度に追加でSP自然回復量 + 1%<br>精錬値が7以上の時、追加でSP自然回復量 + 5%<br>精錬値が9以上の時、追加でSP自然回復量 + 10%",194,1,0];	
	...
	ItemObjNew[5385] = [5385,60,165,100,0,1,100,200,"シェラタン","シエラタン","",243,7,100000000006096,1,100000000006093,1,100265,7,7000012,10,7000079,5,7000077,5,9000194,1,30000009005583,1,0];
	...
	ItemObjNew[5392] = [5392,60,161,100,0,1,100,200,"ストライプホーリーローブ","ストライフホオリイロオフ","",243,7,100000000006028,1,100000000006033,1,100000000006039,1,100265,7,7000012,10,7000079,5,7000077,5,9000194,1,9011483,100,9011488,100,0];
