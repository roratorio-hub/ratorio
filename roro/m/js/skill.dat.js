(function () {

	window.SERE_SUPPORT_SKILL_ID_PYRO_TECHNIC		= 1;
	window.SERE_SUPPORT_SKILL_ID_HEATER			= 4;
	window.SERE_SUPPORT_SKILL_ID_FIRE_CLOAK		= 5;
	window.SERE_SUPPORT_SKILL_ID_AQUA_PLAY			= 10;
	window.SERE_SUPPORT_SKILL_ID_COOLER			= 13;
	window.SERE_SUPPORT_SKILL_ID_WATER_DROP		= 14;
	window.SERE_SUPPORT_SKILL_ID_WATER_BARRIER		= 17;
	window.SERE_SUPPORT_SKILL_ID_GUST				= 19;
	window.SERE_SUPPORT_SKILL_ID_WIND_STEP			= 20;
	window.SERE_SUPPORT_SKILL_ID_BLAST				= 22;
	window.SERE_SUPPORT_SKILL_ID_WIND_CURTAIN		= 23;
	window.SERE_SUPPORT_SKILL_ID_ZEPHYR			= 26;
	window.SERE_SUPPORT_SKILL_ID_PETROLOGY			= 28;
	window.SERE_SUPPORT_SKILL_ID_SOLID_SKIN		= 29;
	window.SERE_SUPPORT_SKILL_ID_CURSED_SOIL		= 31;
	window.SERE_SUPPORT_SKILL_ID_STONE_SHIELD		= 32;
	window.SERE_SUPPORT_SKILL_ID_POWER_OF_GAIA		= 35;
	window.SERE_SUPPORT_SKILL_ID_FLAME_TECHNIQUE 	= 37;
	window.SERE_SUPPORT_SKILL_ID_FLAME_ARMOR 		= 38;
	window.SERE_SUPPORT_SKILL_ID_COLD_FORCE 		= 40;
	window.SERE_SUPPORT_SKILL_ID_CRYSTAL_ARMOR 	= 41;
	window.SERE_SUPPORT_SKILL_ID_GRACE_BREEZE 		= 43;
	window.SERE_SUPPORT_SKILL_ID_EYES_OF_STORM 	= 44;
	window.SERE_SUPPORT_SKILL_ID_EARTH_CARE 		= 46;
	window.SERE_SUPPORT_SKILL_ID_STRONG_PROTECTION = 47;
	window.SERE_SUPPORT_SKILL_ID_DEEP_POISONING 	= 49;
	window.SERE_SUPPORT_SKILL_ID_POISON_SHIELD 	= 50;

	window.SKILL_ID_TEIOAPUCHAGI = 339;
	window.SKILL_ID_RASETSU_HAOGEKI = 617;

/*
	上の２つは例外ですが
	他のスキルは全て skill.dat.js でスキル ID 定数を定義したとしても
	最終的に CSkillManager.js で定数が上書きされてしまうので
	ここで定数定義する意味が無いように見えます

	TODO:
	影響範囲の見極めが必要なので今は削除を見送りますが
	削除可能性について検討したいと思います
*/
/*
	SKILL_ID_TUZYO_KOGEKI = 0;
	SKILL_ID_OKYU_TEATE = 1;
	SKILL_ID_SHINDAFURI = 2;
	SKILL_ID_KEN_SHUREN = 3;
	SKILL_ID_RYOUTKEN_SHUREN = 4;
	SKILL_ID_HP_KAIFUKURYOKU_KOZYO = 5;
	SKILL_ID_BASH = 6;
	SKILL_ID_MAGNUM_BREAK = 7;
	SKILL_ID_PROVOKE = 8;
	SKILL_ID_ENDURE = 9;
	SKILL_ID_IDOZI_HP_KAIFUKU = 10;
	SKILL_ID_KYUSHO_KOGEKI = 11;
	SKILL_ID_AUTO_BERSERK = 12;
	SKILL_ID_DOUBLE_ATTACK = 13;
	SKILL_ID_KAIHIRITSU_ZOKA = 14;
	SKILL_ID_STEAL = 15;
	SKILL_ID_HIDING = 16;
	SKILL_ID_ENVENOM = 17;
	SKILL_ID_GEDOKU = 18;
	SKILL_ID_SUNAMAKI = 19;
	SKILL_ID_BACKSTEP = 20;
	SKILL_ID_ISHIHIROI = 21;
	SKILL_ID_ISHINAGE = 22;
	SKILL_ID_DIVINE_PROTECTION = 23;
	SKILL_ID_DEMON_BANE = 24;
	SKILL_ID_HEAL = 25;
	SKILL_ID_CURE = 26;
	SKILL_ID_SOKUDO_ZOKA = 27;
	SKILL_ID_SOKUDO_GENSHO = 28;
	SKILL_ID_SIGNUM_CRUCIS = 29;
	SKILL_ID_ANGELUS = 30;
	SKILL_ID_BLESSING = 31;
	SKILL_ID_PNEUMA = 32;
	SKILL_ID_AQUA_BENEDICTA = 33;
	SKILL_ID_RUWACH = 34;
	SKILL_ID_TELEPORT = 35;
	SKILL_ID_WARP_PORTAL = 36;
	SKILL_ID_HOLY_LIGHT = 37;
	SKILL_ID_FUKURONO_ME = 38;
	SKILL_ID_WASHINO_ME = 39;
	SKILL_ID_DOUBLE_STRAFING = 40;
	SKILL_ID_ARROW_SHOWER = 41;
	SKILL_ID_SHUCHURYOKU_KOZYO = 42;
	SKILL_ID_YA_SAKUSEI = 43;
	SKILL_ID_CHARGE_ARROW = 44;
	SKILL_ID_SP_KAIFUKURYOKU_KOZYO = 45;
	SKILL_ID_NAPALM_BEAT = 46;
	SKILL_ID_SOUL_STRIKE = 47;
	SKILL_ID_SAFETY_WALL = 48;
	SKILL_ID_STONE_CURSE = 49;
	SKILL_ID_SIGHT = 50;
	SKILL_ID_FIRE_BOLT = 51;
	SKILL_ID_FIRE_BALL = 52;
	SKILL_ID_FIRE_WALL = 53;
	SKILL_ID_COLD_BOLT = 54;
	SKILL_ID_FROST_DIVER = 55;
	SKILL_ID_LIGHTNING_BOLT = 56;
	SKILL_ID_THUNDER_STORM = 57;
	SKILL_ID_ENERGY_COAT = 58;
	SKILL_ID_SHOZIGENKAIRYO_ZOKA = 59;
	SKILL_ID_DISCOUNT = 60;
	SKILL_ID_OVER_CHARGE = 61;
	SKILL_ID_PUSH_CART = 62;
	SKILL_ID_ITEM_KANTE = 63;
	SKILL_ID_ROTEN_KAISETSU = 64;
	SKILL_ID_MAMMONITE = 65;
	SKILL_ID_CART_REVOLUTION = 66;
	SKILL_ID_CHANGE_CART = 67;
	SKILL_ID_LOUD_VOICE = 68;
	SKILL_ID_YARI_SHUREN = 69;
	SKILL_ID_PIERCE = 70;
	SKILL_ID_SPEAR_STUB = 71;
	SKILL_ID_SPEAR_BOOMERANG = 72;
	SKILL_ID_BRANDISH_SPEAR = 73;
	SKILL_ID_TWOHAND_QUICKEN = 74;
	SKILL_ID_AUTO_COUNTER = 75;
	SKILL_ID_BOWLING_BASH = 76;
	SKILL_ID_RIDING = 77;
	SKILL_ID_KIHE_SHUREN = 78;
	SKILL_ID_MIGITE_SHUREN = 79;
	SKILL_ID_HIDARITE_SHUREN = 80;
	SKILL_ID_KATAR_SHUREN = 81;
	SKILL_ID_CLOAKING = 82;
	SKILL_ID_SONIC_BLOW = 83;
	SKILL_ID_GRIM_TOOTH = 84;
	SKILL_ID_ENCHANT_POISON = 85;
	SKILL_ID_POISON_REACT = 86;
	SKILL_ID_VENOM_DUST = 87;
	SKILL_ID_VENOM_SPLASHER = 88;
	SKILL_ID_MACE_SHUREN = 89;
	SKILL_ID_IMPOSITIO_MANUS = 90;
	SKILL_ID_SUFFRAGIUM = 91;
	SKILL_ID_ASPERSIO = 92;
	SKILL_ID_SEITAI_KOFUKU = 93;
	SKILL_ID_SANCTUARY = 94;
	SKILL_ID_RECOVERY = 95;
	SKILL_ID_SLOW_POISON = 96;
	SKILL_ID_RESURRECTION = 97;
	SKILL_ID_KYRIE_ELEISON = 98;
	SKILL_ID_MAGNIFICAT = 99;
	SKILL_ID_GLORIA = 100;
	SKILL_ID_LEX_DIVINA = 101;
	SKILL_ID_TURN_UNDEAD = 102;
	SKILL_ID_LEX_AETERNA = 103;
	SKILL_ID_MAGNUS_EXORCISMUS = 104;
	SKILL_ID_SKID_TRAP = 105;
	SKILL_ID_LAND_MINE = 106;
	SKILL_ID_ANKLESNARE = 107;
	SKILL_ID_FLASHER = 108;
	SKILL_ID_SHOCKWAVE_TRAP = 109;
	SKILL_ID_SANDMAN = 110;
	SKILL_ID_FREEZING_TRAP = 111;
	SKILL_ID_BLAST_MINE = 112;
	SKILL_ID_CLAYMORE_TRAP = 113;
	SKILL_ID_REMOVE_TRAP = 114;
	SKILL_ID_TALKIE_BOX = 115;
	SKILL_ID_BEAST_BANE = 116;
	SKILL_ID_FALCON_MASTERY = 117;
	SKILL_ID_BLITZ_BEAT = 118;
	SKILL_ID_STEEL_CROW = 119;
	SKILL_ID_DETECTING = 120;
	SKILL_ID_SPRING_TRAP = 121;
	SKILL_ID_FIRE_PILLAR = 122;
	SKILL_ID_MONSTER_ZYOHO = 123;
	SKILL_ID_SIGHT_RASHER = 124;
	SKILL_ID_METEOR_STORM = 125;
	SKILL_ID_JUPITER_THUNDER = 126;
	SKILL_ID_LORD_OF_VERMILLION = 127;
	SKILL_ID_WATER_BALL = 128;
	SKILL_ID_ICE_WALL = 129;
	SKILL_ID_FROST_NOVA = 130;
	SKILL_ID_STORM_GUST = 131;
	SKILL_ID_EARTH_SPIKE = 132;
	SKILL_ID_HEAVENS_DRIVE = 133;
	SKILL_ID_QUAGMIRE = 134;
	SKILL_ID_TETSU_SEIZO = 135;
	SKILL_ID_KOTETSU_SEIZO = 136;
	SKILL_ID_ZOKUSEISEKI_SEIZO = 137;
	SKILL_ID_ORIDEOCON_KENKYU = 138;
	SKILL_ID_TANKEN_SEISAKU = 139;
	SKILL_ID_KEN_SEISAKU = 140;
	SKILL_ID_RYOTEKEN_SEISAKU = 141;
	SKILL_ID_ONO_SEISAKU = 142;
	SKILL_ID_MACE_SEISAKU = 143;
	SKILL_ID_KNUCKLE_SEISAKU = 144;
	SKILL_ID_YARI_SEISAKU = 145;
	SKILL_ID_HILT_BINDING = 146;
	SKILL_ID_KOSEKI_HAKKEN = 147;
	SKILL_ID_BUKI_KENKYU = 148;
	SKILL_ID_BUKI_SHURI = 149;
	SKILL_ID_SKIN_TEMPERING = 150;
	SKILL_ID_HAMMER_FALL = 151;
	SKILL_ID_ADRENALINE_RUSH = 152;
	SKILL_ID_WEAPON_PERFECTION = 153;
	SKILL_ID_OVER_TRUST = 154;
	SKILL_ID_MAXIMIZE_POWER = 155;
	SKILL_ID_FAITH = 156;
	SKILL_ID_AUTO_GUARD_OLD = 157;
	SKILL_ID_SHIELD_CHARGE = 158;
	SKILL_ID_SHIELD_BOOMERANG = 159;
	SKILL_ID_REFLECT_SHIELD = 160;
	SKILL_ID_HOLY_CROSS = 161;
	SKILL_ID_GRAND_CROSS = 162;
	SKILL_ID_DEBOTION = 163;
	SKILL_ID_PROVIDENCE = 164;
	SKILL_ID_DEFENDER = 165;
	SKILL_ID_SPEAR_QUICKEN = 166;
	SKILL_ID_SNATCHER = 167;
	SKILL_ID_STEAL_COIN = 168;
	SKILL_ID_BACK_STAB = 169;
	SKILL_ID_TUNNEL_DRIVE = 170;
	SKILL_ID_SURPRISE_ATTACK = 171;
	SKILL_ID_STRIP_WEAPON = 172;
	SKILL_ID_STRIP_SHIELD = 173;
	SKILL_ID_STRIP_ARMER = 174;
	SKILL_ID_STRIP_HELM = 175;
	SKILL_ID_INTIMIDATE = 176;
	SKILL_ID_GRAPHITY = 177;
	SKILL_ID_FLAG_GRAPHITY = 178;
	SKILL_ID_CLEANER = 179;
	SKILL_ID_GANGSTAR_PARADISE = 180;
	SKILL_ID_COMPULSION_DISCOUNT = 181;
	SKILL_ID_CLONE_SKILL = 182;
	SKILL_ID_TEKKEN = 183;
	SKILL_ID_IBUKI = 184;
	SKILL_ID_KIKO = 185;
	SKILL_ID_KIDATSU = 186;
	SKILL_ID_SANDANSHO = 187;
	SKILL_ID_RENDASHO = 188;
	SKILL_ID_MORYUKEN = 189;
	SKILL_ID_ZANEI = 190;
	SKILL_ID_MIKIRI = 191;
	SKILL_ID_SHIDAN = 192;
	SKILL_ID_HAKKEI = 193;
	SKILL_ID_SHIRAHADORI = 194;
	SKILL_ID_BAKURETSU_HADO = 195;
	SKILL_ID_KONGO = 196;
	SKILL_ID_ASHURA_HAOKEN = 197;
	SKILL_ID_GAKKINO_RENSHU = 198;
	SKILL_ID_MUSICAL_STRIKE = 199;
	SKILL_ID_FUKYOWAON = 200;
	SKILL_ID_SAMUI_JOKE = 201;
	SKILL_ID_KUCHIBUE = 202;
	SKILL_ID_YUHINO_ASSASINCROSS = 203;
	SKILL_ID_BRAGINO_UTA = 204;
	SKILL_ID_IDUNNNO_RINGO = 205;
	SKILL_ID_DANCENO_RENSHU = 206;
	SKILL_ID_YAUCHI = 207;
	SKILL_ID_ZIBUNKATTENA_DANCE = 208;
	SKILL_ID_SCREAM = 209;
	SKILL_ID_HUMMING = 210;
	SKILL_ID_WATASHIWO_WASURENAIDE = 211;
	SKILL_ID_KOUNNO_KISS = 212;
	SKILL_ID_SERVICE_FOR_YOU = 213;
	SKILL_ID_ADLIB = 214;
	SKILL_ID_ENCORE = 215;
	SKILL_ID_KOMORIUTA = 216;
	SKILL_ID_NJORDNO_UTAGE = 217;
	SKILL_ID_EIENNO_KONTON = 218;
	SKILL_ID_IKUSADAIKONO_HIBIKI = 219;
	SKILL_ID_NIBELUGENNO_YUBIWA = 220;
	SKILL_ID_LOKINO_SAKEBI = 221;
	SKILL_ID_SHINENNO_NAKANI = 222;
	SKILL_ID_FUZIMINO_SIEGFRIED = 223;
	SKILL_ID_ADVANCED_BOOK = 224;
	SKILL_ID_CAST_CANCEL = 225;
	SKILL_ID_MAGIC_ROD = 226;
	SKILL_ID_SPELL_BREAKER = 227;
	SKILL_ID_FREE_CAST = 228;
	SKILL_ID_AUTO_MAGICIAN_SPELL = 229;
	SKILL_ID_FLAME_LAUNCHER = 230;
	SKILL_ID_FROST_WEAPON = 231;
	SKILL_ID_LIGHTNING_LOADER = 232;
	SKILL_ID_SEISMIC_WEAPON = 233;
	SKILL_ID_DRAGONOLOGY = 234;
	SKILL_ID_VOLCANO = 235;
	SKILL_ID_DELUGE = 236;
	SKILL_ID_VIOLENT_GALE = 237;
	SKILL_ID_LAND_PROTECTOR = 238;
	SKILL_ID_DISPELL = 239;
	SKILL_ID_ABRACADABRA = 240;
	SKILL_ID_ONO_SHUREN = 241;
	SKILL_ID_LEARNING_POTION = 242;
	SKILL_ID_PHARMACY = 243;
	SKILL_ID_ACID_TERROR = 244;
	SKILL_ID_POTION_PITCHER = 245;
	SKILL_ID_BIOPLANT = 246;
	SKILL_ID_SPHERE_MINE = 247;
	SKILL_ID_DEMONSTRATION = 248;
	SKILL_ID_CHEMICAL_WEAPON_CHARGE = 249;
	SKILL_ID_CHEMICAL_SHIELD_CHARGE = 250;
	SKILL_ID_CHEMICAL_ARMER_CHARGE = 251;
	SKILL_ID_CHEMICAL_HELM_CHARGE = 252;
	SKILL_ID_BAKURETSU_HADO_SUPER_NOVICE = 253;
	SKILL_ID_AURA_BLADE = 254;
	SKILL_ID_PARIYING = 255;
	SKILL_ID_CONCENTRATION = 256;
	SKILL_ID_TENTION_RELAX = 257;
	SKILL_ID_BERSERK = 258;
	SKILL_ID_SPIRAL_PIERCE = 259;
	SKILL_ID_HEAD_CRUSH = 260;
	SKILL_ID_JOINT_BEAT = 261;
	SKILL_ID_KATAR_KENKYU = 262;
	SKILL_ID_SOUL_BREAKER = 263;
	SKILL_ID_METEOR_ASSALT = 264;
	SKILL_ID_CREATE_DEADLY_POISON = 265;
	SKILL_ID_ENCHANT_DEADLY_POISON = 266;
	SKILL_ID_ASSUMPTIO = 267;
	SKILL_ID_BASILICA = 268;
	SKILL_ID_MEDITATIO = 269;
	SKILL_ID_TRUE_SIGHT = 270;
	SKILL_ID_FALCON_ASSALT = 271;
	SKILL_ID_SHARP_SHOOTING = 272;
	SKILL_ID_WIND_WALK = 273;
	SKILL_ID_SOUL_DRAIN = 274;
	SKILL_ID_MAGIC_CRUSHER = 275;
	SKILL_ID_MAHORYOKU_ZOFUKU = 276;
	SKILL_ID_NAPALM_VULKAN = 277;
	SKILL_ID_MELTDOWN = 278;
	SKILL_ID_OKANE_SEIZO = 279;
	SKILL_ID_KATAMARI_SEIZO = 280;
	SKILL_ID_CART_BOOST_WS = 281;
	SKILL_ID_UNMEINO_TALOTCARD = 282;
	SKILL_ID_PRESSURE = 283;
	SKILL_ID_SACRIFICE = 284;
	SKILL_ID_GOSPEL = 285;
	SKILL_ID_CHASEWALK = 286;
	SKILL_ID_REJECT_SWORD = 287;
	SKILL_ID_MOKOKOHAZAN = 288;
	SKILL_ID_BUKKOKEN = 289;
	SKILL_ID_RENCHUHOGEKI = 290;
	SKILL_ID_SOUL_COLECT = 291;
	SKILL_ID_ARRAW_VULKAN = 292;
	SKILL_ID_RENKIKO = 293;
	SKILL_ID_MARIONET_CONTROL = 294;
	SKILL_ID_LIFE_CONVERSION = 295;
	SKILL_ID_SOUL_CHANGE = 296;
	SKILL_ID_SOUL_BURN = 297;
	SKILL_ID_MIND_BREAKER = 298;
	SKILL_ID_ALCHEMY = 299;
	SKILL_ID_POTION_SYNAPSE = 300;
	SKILL_ID_SANDAN_DELAY_ZOKA = 301;
	SKILL_ID_TOMAHAWKNAGE = 302;
	SKILL_ID_PULSE_STRIKE = 303;
	SKILL_ID_BERSERK_PITCHER = 304;
	SKILL_ID_TEIOAPUCHAGI_IN_DASH = 305;
	SKILL_ID_VENOM_KNIFE = 306;
	SKILL_ID_FANTASMIC_ARROW = 307;
	SKILL_ID_CHARGE_ATTACK = 308;
	SKILL_ID_SUPER_NOVICE_NODEAD_BONUS = 309;
	SKILL_ID_MARIAGE_STATUS = 310;
	SKILL_ID_SKILL_COUNT_CREATE_ARMS_MASTER = 311;
	SKILL_ID_DARK_STRIKE = 312;
	SKILL_ID_313 = 313;
	SKILL_ID_314 = 314;
	SKILL_ID_315 = 315;
	SKILL_ID_316 = 316;
	SKILL_ID_NUKUMORI = 317;
	SKILL_ID_NUKUMORI_KABE = 318;
	SKILL_ID_HEAVENS_DRIVE_FOR_CLONE = 319;
	SKILL_ID_WATER_BALL_FOR_CLONE = 320;
	SKILL_ID_ASHURA_HAOKEN_SPKOTEI = 321;
	SKILL_ID_MEMORIZE = 322;
	SKILL_ID_323 = 323;
	SKILL_ID_SHIELD_CHAIN = 324;
	SKILL_ID_GRAVITATION_FIELD = 325;
	SKILL_ID_CART_TERMINATION = 326;
	SKILL_ID_OVER_TRUST_MAX = 327;
	SKILL_ID_ACID_DEMONSTRATION = 328;
	SKILL_ID_TAIRIGI = 329;
	SKILL_ID_FEORICHAGINO_KAMAE = 330;
	SKILL_ID_FEORICHAGI = 331;
	SKILL_ID_NERYOCHAGINO_KAMAE = 332;
	SKILL_ID_NERYOCHAGI = 333;
	SKILL_ID_TORURYOCHAGINO_KAMAE = 334;
	SKILL_ID_TORURYOCHAGI = 335;
	SKILL_ID_APUCHAORURIGINO_KAMAE = 336;
	SKILL_ID_APUCHAORURIGI = 337;
	SKILL_ID_RAKHO = 338;
	SKILL_ID_ODAYAKANA_KYUSOKU = 340;
	SKILL_ID_TANOSHI_KYUSOKU = 341;
	SKILL_ID_FIGHT = 342;
	SKILL_ID_NOPITIGI = 343;
	SKILL_ID_TAEGWON_MISSION = 344;
	SKILL_ID_TAEGWON_RANKER = 345;
	SKILL_ID_ATATAKAI_KAZE = 346;
	SKILL_ID_TAIYOTO_TSUKITO_HOSHINO_KANZYO = 347;
	SKILL_ID_TAIYONO_NUKUMORI = 348;
	SKILL_ID_TSUKINO_NUKUMORI = 349;
	SKILL_ID_HOSHINO_NUKUMORI = 350;
	SKILL_ID_TAIYOTO_TSUKITO_HOSHINO_NIKUSHIMI = 351;
	SKILL_ID_TAIYONO_IKARI = 352;
	SKILL_ID_TSUKINO_IKARI = 353;
	SKILL_ID_HOSHINO_IKARI = 354;
	SKILL_ID_TAIYONO_ANRAKU = 355;
	SKILL_ID_TSUKINO_ANRAKU = 356;
	SKILL_ID_HOSHINO_ANRAKU = 357;
	SKILL_ID_TAIYONO_SHUKUFUKU = 358;
	SKILL_ID_TSUKUNO_SHUKUFUKU = 359;
	SKILL_ID_HOSHINO_SHUKUFUKU = 360;
	SKILL_ID_TAIYOTO_TSUKITO_HOSHINO_AKUMA = 361;
	SKILL_ID_TAIYOTO_TSUKITO_HOSHINO_TOMO = 362;
	SKILL_ID_TAIYOTO_TSUKITO_HOSHINO_CHISHIKI = 363;
	SKILL_ID_TAIYOTO_TSUKITO_HOSHINO_YUGO = 364;
	SKILL_ID_TAIYOTO_TSUKITO_HOSHINO_KISEKI = 365;
	SKILL_ID_TAIYOTO_TSUKITO_HOSHINO_TENSHI = 366;
	SKILL_ID_SHUKUFUKU = 367;
	SKILL_ID_KAISEL = 368;
	SKILL_ID_KAAHI = 369;
	SKILL_ID_KAUPU = 370;
	SKILL_ID_KAITO = 371;
	SKILL_ID_KAINA = 372;
	SKILL_ID_ESTIN = 373;
	SKILL_ID_ESTON = 374;
	SKILL_ID_ESMA = 375;
	SKILL_ID_ESU = 376;
	SKILL_ID_ESKA = 377;
	SKILL_ID_ESKU = 378;
	SKILL_ID_SPURT_ZYOTAI = 379;
	SKILL_ID_ZIBUNIGAINO_PTNINZU_FOR_FIGHT = 380;
	SKILL_ID_SONIC_ACCELERATION = 381;
	SKILL_ID_SUNKEI = 382;
	SKILL_ID_CLOSE_CONFINE = 383;
	SKILL_ID_SHIELD_BOOMERANG_TAMASHI = 384;
	SKILL_ID_SUPER_NOVICENO_TAMASHI = 385;
	SKILL_ID_ONEHAND_QUICKEN = 386;
	SKILL_ID_HOLY_LIGHT_TAMASHI = 387;
	SKILL_ID_SONIC_BLOW_TAMASHI = 388;
	SKILL_ID_FULL_ADRENALINE_RUSH = 389;
	SKILL_ID_HUNTERNO_TAMASHI_KOKA = 390;
	SKILL_ID_BEAST_STRAIFING = 391;
	SKILL_ID_TENSE_ICHIZISHOKUNO_TAMASHI = 392;
	SKILL_ID_TOTEKI_SHUREN = 393;
	SKILL_ID_SHURIKEN_NAGE = 394;
	SKILL_ID_KUNAI_NAGE = 395;
	SKILL_ID_FUMASHURIKEN_NAGE = 396;
	SKILL_ID_ZENI_NAGE = 397;
	SKILL_ID_TATAMI_GAESHI = 398;
	SKILL_ID_KAGETOBI = 399;
	SKILL_ID_KASUMIGIRI = 400;
	SKILL_ID_KAGEKIRI = 401;
	SKILL_ID_UTSUSEMI = 402;
	SKILL_ID_KAGEBUNSHIN = 403;
	SKILL_ID_NEN = 404;
	SKILL_ID_ISSEN = 405;
	SKILL_ID_NINPO_SHUREN = 406;
	SKILL_ID_KOUENKA = 407;
	SKILL_ID_KAENZIN = 408;
	SKILL_ID_RYUENZIN = 409;
	SKILL_ID_HYOSENSO = 410;
	SKILL_ID_SUITON = 411;
	SKILL_ID_TSURARAOTOSHI = 412;
	SKILL_ID_FUZIN = 413;
	SKILL_ID_RAIGEKISAI = 414;
	SKILL_ID_SAKUFU = 415;
	SKILL_ID_COUNT_OF_COIN = 416;
	SKILL_ID_FLYING = 417;
	SKILL_ID_TRIPLE_ACTION = 418;
	SKILL_ID_BULLS_EYE = 419;
	SKILL_ID_MADNESSS_CANCELER = 420;
	SKILL_ID_ADJUSTMENT = 421;
	SKILL_ID_INCREASING_ACCURACY = 422;
	SKILL_ID_MAGICAL_BARRET = 423;
	SKILL_ID_CRACKER = 424;
	SKILL_ID_SINGLE_ACTION = 425;
	SKILL_ID_SNAKE_EYE = 426;
	SKILL_ID_CHAIN_ACTION = 427;
	SKILL_ID_RAPID_SHOWER = 428;
	SKILL_ID_DEATHPERAD = 429;
	SKILL_ID_TRACKING = 430;
	SKILL_ID_DISARM = 431;
	SKILL_ID_PIERCING_SHOT = 432;
	SKILL_ID_GATLING_FEVER = 433;
	SKILL_ID_DUST = 434;
	SKILL_ID_FULL_BASTER = 435;
	SKILL_ID_SPREAD_ATTACK = 436;
	SKILL_ID_GROUND_DRIFT = 437;
	SKILL_ID_ISSEN_MAX = 438;
	SKILL_ID_ENCHANT_BLADE = 439;
	SKILL_ID_SONIC_WAVE = 440;
	SKILL_ID_DEATH_BOUND = 441;
	SKILL_ID_HANDRED_SPEAR = 442;
	SKILL_ID_WIND_CUTTER = 443;
	SKILL_ID_PHANTOM_SLAST = 444;
	SKILL_ID_IGNITION_BREAK = 445;
	SKILL_ID_DRAGON_TRAINING = 446;
	SKILL_ID_FIRE_DRAGON_BREATH = 447;
	SKILL_ID_DRAGON_HOWLING = 448;
	SKILL_ID_RUNE_MASTERY = 449;
	SKILL_ID_GIANT_GROWTH = 450;
	SKILL_ID_VITARITY_ACTIVATION = 451;
	SKILL_ID_STORM_BLAST = 452;
	SKILL_ID_STONE_HARD_SKIN = 453;
	SKILL_ID_FIGHTING_SPIRIT = 454;
	SKILL_ID_AVANDANCE = 455;
	SKILL_ID_CRUSH_STRIKE = 456;
	SKILL_ID_REFRESH = 457;
	SKILL_ID_MILLENNIUM_SHIELD = 458;
	SKILL_ID_VENOM_IMPRESS = 459;
	SKILL_ID_CROSS_IMPACT = 460;
	SKILL_ID_DARK_ILLUSION = 461;
	SKILL_ID_SHINDOKU_KENKYU = 462;
	SKILL_ID_SHINDOKU_SEIZO = 463;
	SKILL_ID_ANTIDOTE = 464;
	SKILL_ID_POISONING_WEAPON = 465;
	SKILL_ID_VENOM_PRESSURE = 466;
	SKILL_ID_POISON_SMOKE = 467;
	SKILL_ID_WEAPON_BLOCKING = 468;
	SKILL_ID_COUNTER_SLASH = 469;
	SKILL_ID_WEAPON_CRUSH = 470;
	SKILL_ID_CLOAKING_EXCEED = 471;
	SKILL_ID_PHANTOM_MENUS = 472;
	SKILL_ID_HALLUCINATION_WALK = 473;
	SKILL_ID_ROLLING_CUTTER = 474;
	SKILL_ID_CROSS_RIPPER_SLASHER = 475;
	SKILL_ID_JUDEX = 476;
	SKILL_ID_ANCILLA = 477;
	SKILL_ID_ADORAMUS = 478;
	SKILL_ID_CLEMENTIA = 479;
	SKILL_ID_CANTOCANDIDUS = 480;
	SKILL_ID_COLUCEO_HEAL = 481;
	SKILL_ID_EPICLESIS = 482;
	SKILL_ID_PRAEFATIO = 483;
	SKILL_ID_ORATIO = 484;
	SKILL_ID_LAUDAAGNUS = 485;
	SKILL_ID_LAUDARAMUS = 486;
	SKILL_ID_EUCHARISTICA = 487;
	SKILL_ID_RENOVATIO = 488;
	SKILL_ID_HIGHNESS_HEAL = 489;
	SKILL_ID_CLEARANCE = 490;
	SKILL_ID_EXPIATIO = 491;
	SKILL_ID_DUPLELIGHT = 492;
	SKILL_ID_SILENTIUM = 493;
	SKILL_ID_SECRAMENT = 494;
	SKILL_ID_RANGER_MAIN = 495;
	SKILL_ID_CAMOUFLAGE = 496;
	SKILL_ID_AIMED_BOLT = 497;
	SKILL_ID_ARROW_STORM = 498;
	SKILL_ID_FEAR_BLEATH = 499;
	SKILL_ID_TRAP_KENKYU = 500;
	SKILL_ID_MAGENTA_TRAP = 501;
	SKILL_ID_COBALT_TRAP = 502;
	SKILL_ID_VERDURE_TRAP = 503;
	SKILL_ID_MAZE_TRAP = 504;
	SKILL_ID_CLUSTER_BOMB = 505;
	SKILL_ID_DETONATOR = 506;
	SKILL_ID_FIRING_TRAP = 507;
	SKILL_ID_ICEBOUND_TRAP = 508;
	SKILL_ID_ELECTRIC_SHOCKER = 509;
	SKILL_ID_WUG_MASTERY = 510;
	SKILL_ID_WUG_BITE = 511;
	SKILL_ID_TOOTH_OF_WUG = 512;
	SKILL_ID_WUG_STRIKE = 513;
	SKILL_ID_EIBINNA_KYUKAKU = 514;
	SKILL_ID_WUG_RIDER = 515;
	SKILL_ID_WUG_DASH = 516;
	SKILL_ID_WHITE_IN_PRISON = 517;
	SKILL_ID_SOUL_EXPANSION = 518;
	SKILL_ID_FROST_MISTY = 519;
	SKILL_ID_JACK_FROST = 520;
	SKILL_ID_MARSH_OF_ABYSS = 521;
	SKILL_ID_RECOGNIZED_SPELL = 522;
	SKILL_ID_SIENNA_EXEXRATE = 523;
	SKILL_ID_RADIUS = 524;
	SKILL_ID_STASIS = 525;
	SKILL_ID_DRAIN_LIFE = 526;
	SKILL_ID_CRYMSON_ROCK = 527;
	SKILL_ID_HELL_INFERNO = 528;
	SKILL_ID_COMMET = 529;
	SKILL_ID_CHAIN_LIGHTNING = 530;
	SKILL_ID_EARTH_STRAIN = 531;
	SKILL_ID_TETRA_BOLTEX = 532;
	SKILL_ID_SUMMON_FIRE_BALL = 533;
	SKILL_ID_SUMMON_WATER_BALL = 534;
	SKILL_ID_SUMMON_LIGHTNING_BALL = 535;
	SKILL_ID_SUMMON_STONE = 536;
	SKILL_ID_RELEASE = 537;
	SKILL_ID_READING_SPELLBOOK = 538;
	SKILL_ID_FREEZING_SPELL = 539;
	SKILL_ID_ONO_SHUREN_MECHANIC = 540;
	SKILL_ID_AXE_TORNADE = 541;
	SKILL_ID_AXE_BOOMERANG = 542;
	SKILL_ID_POWER_SWING = 543;
	SKILL_ID_HITO_DAICHINO_KENKYU = 544;
	SKILL_ID_FAW_SILVER_SNIPER = 545;
	SKILL_ID_FAW_MAGIC_DECOY = 546;
	SKILL_ID_FAW_KAIZYO = 547;
	SKILL_ID_MADOGEAR_LICENSE = 548;
	SKILL_ID_BOOST_KNUCKLE = 549;
	SKILL_ID_PILE_BUNKER = 550;
	SKILL_ID_VULCAN_ARM = 551;
	SKILL_ID_FLAME_THROWER = 552;
	SKILL_ID_COLD_THROWER = 553;
	SKILL_ID_ARMS_CANNON = 554;
	SKILL_ID_ACCELARATION = 555;
	SKILL_ID_HOVERING = 556;
	SKILL_ID_FRONTSIDE_SLIDE = 557;
	SKILL_ID_REARSIDE_SLIDE = 558;
	SKILL_ID_MAINFRAME_KAIZO = 559;
	SKILL_ID_SHAPE_SHIFT = 560;
	SKILL_ID_INFRARED_SCAN = 561;
	SKILL_ID_ANALYZE = 562;
	SKILL_ID_SELF_DESTRUCTION = 563;
	SKILL_ID_EMERGENCY_COOL = 564;
	SKILL_ID_MAGNETIC_FIELD = 565;
	SKILL_ID_NUTRAL_BARRIER = 566;
	SKILL_ID_STEALTH_FIELD = 567;
	SKILL_ID_REPEAR = 568;
	SKILL_ID_CANNON_SPEAR = 569;
	SKILL_ID_BANISHING_POINT = 570;
	SKILL_ID_TRUMPLE = 571;
	SKILL_ID_SHIELD_PRESS = 572;
	SKILL_ID_REFLECT_DAMAGE = 573;
	SKILL_ID_PINGPOINT_ATTACK = 574;
	SKILL_ID_FORCE_OF_BANGUARD = 575;
	SKILL_ID_RAGE_BURST_ATTACK = 576;
	SKILL_ID_SHIELD_SPELL = 577;
	SKILL_ID_EXCEED_BREAK = 578;
	SKILL_ID_OVER_BLAND = 579;
	SKILL_ID_PRESTAGE = 580;
	SKILL_ID_BANDING = 581;
	SKILL_ID_MOON_SLUSHER = 582;
	SKILL_ID_RAY_OF_GENESIS = 583;
	SKILL_ID_PIETY = 584;
	SKILL_ID_EARTH_DRIVE = 585;
	SKILL_ID_HESPERUS_SLIT = 586;
	SKILL_ID_INSPIRATION = 587;
	SKILL_ID_BODY_PAINTING = 588;
	SKILL_ID_MASKARADE_INOVATION = 589;
	SKILL_ID_MASKARADE_GLOOMY = 590;
	SKILL_ID_MASKARADE_IGNORANCE = 591;
	SKILL_ID_MASKARADE_RAGENESS = 592;
	SKILL_ID_MASKARADE_WEEKNESS = 593;
	SKILL_ID_MASKARADE_UNLUCKY = 594;
	SKILL_ID_REPORDUCE = 595;
	SKILL_ID_AUTO_SHADOW_SPELL = 596;
	SKILL_ID_SHADOW_FORM = 597;
	SKILL_ID_DEADLY_INEFFECT = 598;
	SKILL_ID_INVISIBILITY = 599;
	SKILL_ID_MANHOLE = 600;
	SKILL_ID_DEMENSION_DOOR = 601;
	SKILL_ID_BLOODY_LAST = 602;
	SKILL_ID_FAINT_BOMB = 603;
	SKILL_ID_CHAOS_PANIC = 604;
	SKILL_ID_MAELSTORM = 605;
	SKILL_ID_FATAL_MENUS = 606;
	SKILL_ID_STRIP_ACCESSARY = 607;
	SKILL_ID_TRIANGLE_SHOT = 608;
	SKILL_ID_SORYUKYAKU = 609;
	SKILL_ID_TENRACHIMO = 610;
	SKILL_ID_ZIRAISHIN = 611;
	SKILL_ID_BAKKISANDAN = 612;
	SKILL_ID_SHURASHINDAN = 613;
	SKILL_ID_DAITENHOSUI = 614;
	SKILL_ID_GOHO = 615;
	SKILL_ID_RASETSU_HAOGEKI_MAX = 616;
	SKILL_ID_SENPUTAI = 618;
	SKILL_ID_ZYUBAKUZIN = 619;
	SKILL_ID_SENDENPO = 620;
	SKILL_ID_SENRYU_SHOTEN = 621;
	SKILL_ID_SISIKO = 622;
	SKILL_ID_RAIKODAN = 623;
	SKILL_ID_TENKETSU_MOKU = 624;
	SKILL_ID_TENKETSU_KAI = 625;
	SKILL_ID_TENKETSU_KYU = 626;
	SKILL_ID_TENKETSU_HAN = 627;
	SKILL_ID_TENKETSU_KATSU = 628;
	SKILL_ID_KYUKIKO = 629;
	SKILL_ID_HASAICHU = 630;
	SKILL_ID_LESSON = 631;
	SKILL_ID_YASURAGINO_KOMORIUTA = 632;
	SKILL_ID_ZIGOKUNO_UTA = 633;
	SKILL_ID_FUKAKUTEYOSONO_GENGO = 634;
	SKILL_ID_MELANCHOLY = 635;
	SKILL_ID_SIRENNO_KOE = 636;
	SKILL_ID_ZYUNKANSURU_SIZENNO_OTO = 637;
	SKILL_ID_SEISHINO_SAKAIDE = 638;
	SKILL_ID_SHINDOZANKYO = 639;
	SKILL_ID_DOMINION_IMPULSE = 640;
	SKILL_ID_METALIC_SOUND = 641;
	SKILL_ID_SEVERE_RAINSTORM = 642;
	SKILL_ID_FUSHANIMUKATTE_TOTSUGEKI = 643;
	SKILL_ID_ECHONO_UTA = 644;
	SKILL_ID_HARMONIZE = 645;
	SKILL_ID_TSUKIAKARINO_SERENADE = 646;
	SKILL_ID_KOIBITOTACHINO_TAMENO_SYMPHONY = 647;
	SKILL_ID_SWING_DANCE = 648;
	SKILL_ID_LERAORNO_TSUYU = 649;
	SKILL_ID_BEYOND_OF_WARCRY = 650;
	SKILL_ID_MANANO_UTA = 651;
	SKILL_ID_MELODY_OF_THINK = 652;
	SKILL_ID_DANCE_WITH_WUG = 653;
	SKILL_ID_FRIDAY_NIGHT_FEVER = 654;
	SKILL_ID_SOUND_OF_DESTRUCTION = 655;
	SKILL_ID_ENDLESS_HUMMING_VOICE = 656;
	SKILL_ID_GREAT_ECHO = 657;
	SKILL_ID_FIRE_WALK = 658;
	SKILL_ID_ELECTRIC_WALK = 659;
	SKILL_ID_SPELL_FIST = 660;
	SKILL_ID_VACUUM_EXTREME = 661;
	SKILL_ID_PSYCHIC_WAVE = 662;
	SKILL_ID_CLOUD_KILL = 663;
	SKILL_ID_POISON_BUSTER = 664;
	SKILL_ID_STRIKING = 665;
	SKILL_ID_EARTH_GRAVE = 666;
	SKILL_ID_DIAMOND_DUST = 667;
	SKILL_ID_WARMER = 668;
	SKILL_ID_VERATURE_SPEAR = 669;
	SKILL_ID_ARRULLO = 670;
	SKILL_ID_SUMMON_AGNI = 671;
	SKILL_ID_SUMMON_AQUA = 672;
	SKILL_ID_SUMMON_VENTOS = 673;
	SKILL_ID_SUMMON_TERA = 674;
	SKILL_ID_ELEMENTAL_CONTROL = 675;
	SKILL_ID_ELEMENTAL_ACTION = 676;
	SKILL_ID_ELEMENTAL_ANALYSIS = 677;
	SKILL_ID_ELEMENTAL_SYMPASY = 678;
	SKILL_ID_ELEMENTAL_CURE = 679;
	SKILL_ID_FIRE_INSIGNIA = 680;
	SKILL_ID_WATER_INSIGNIA = 681;
	SKILL_ID_WIND_INSIGNIA = 682;
	SKILL_ID_EARTH_INSIGNIA = 683;
	SKILL_ID_PILO_TECHNIC = 684;
	SKILL_ID_CIRCLE_OF_FIRE = 685;
	SKILL_ID_FIRE_ARROW = 686;
	SKILL_ID_HEATER = 687;
	SKILL_ID_FIRE_CLOAK = 688;
	SKILL_ID_FIRE_BOMB = 689;
	SKILL_ID_TOROPIC = 690;
	SKILL_ID_FIRE_MANTLE = 691;
	SKILL_ID_FIRE_WAVE = 692;
	SKILL_ID_AQUA_PLAY = 693;
	SKILL_ID_WATER_SCREEN = 694;
	SKILL_ID_ICE_NEEDLE = 695;
	SKILL_ID_COOLER = 696;
	SKILL_ID_WATER_DROP = 697;
	SKILL_ID_WATER_SCREW = 698;
	SKILL_ID_CHILLY_AIR = 699;
	SKILL_ID_WATER_BARRIER = 700;
	SKILL_ID_TAIDAL_WEAPON = 701;
	SKILL_ID_GAST = 702;
	SKILL_ID_WIND_STEP = 703;
	SKILL_ID_WIND_SLASH = 704;
	SKILL_ID_BLAST = 705;
	SKILL_ID_WIND_CURTAIN = 706;
	SKILL_ID_HURRICANE_RAGE = 707;
	SKILL_ID_WILD_STORM = 708;
	SKILL_ID_XEPHER = 709;
	SKILL_ID_TAYPHOON_MISSILE = 710;
	SKILL_ID_PETROLOGY = 711;
	SKILL_ID_SOLID_SKIN = 712;
	SKILL_ID_STONE_HUMMER = 713;
	SKILL_ID_CURSED_SOIL = 714;
	SKILL_ID_STONE_SHIELD = 715;
	SKILL_ID_ROCK_CRUSHER = 716;
	SKILL_ID_UP_HIEBAL = 717;
	SKILL_ID_POWER_OF_GAIA = 718;
	SKILL_ID_STONE_RAIN = 719;
	SKILL_ID_KEN_SHUREN_GENETIC = 720;
	SKILL_ID_CART_KAIZO = 721;
	SKILL_ID_CART_TORNADO = 722;
	SKILL_ID_CART_CANNON = 723;
	SKILL_ID_CART_BOOST_GENETIC = 724;
	SKILL_ID_CHANGE_MATERIAL = 725;
	SKILL_ID_SLING_ITEM = 726;
	SKILL_ID_SPECIAL_PHARMACY = 727;
	SKILL_ID_MIX_COOKING = 728;
	SKILL_ID_BAKUDAN_SEIZO = 729;
	SKILL_ID_THORN_TRAP = 730;
	SKILL_ID_THORN_WALL = 731;
	SKILL_ID_CRAZY_WEED = 732;
	SKILL_ID_BLOOD_SUCKER = 733;
	SKILL_ID_HELLS_PLANT = 734;
	SKILL_ID_HOWLING_OF_MANDRAGORA = 735;
	SKILL_ID_SPORE_EXPLOSION = 736;
	SKILL_ID_DEMONIC_FIRE = 737;
	SKILL_ID_FIRE_EXPANSION = 738;
	SKILL_ID_MADOGEAR = 739;
	SKILL_ID_SELF_DESTRUCTION_MAX = 740;
	SKILL_ID_GRAHAM_LIGHT = 741;
	SKILL_ID_MIRIAM_LIGHT = 742;
	SKILL_ID_MAGIC_SETTING_FOR_AUTO_SPELL = 743;
	SKILL_ID_SHIELD_SPELL_ATK_PLUS = 744;
	SKILL_ID_SHIELD_SPELL_DEF_PLUS = 745;
	SKILL_ID_SKILL_LV_DEFENDER_FOR_PRESTAGE = 746;
	SKILL_ID_MAGIC_SETTING_FOR_AUTO_SHADOW_SPELL = 747;
	SKILL_ID_ZENKI_CHUNYU = 748;
	SKILL_ID_SAGENO_TAMASHI_MAHONO_SHUTOKU_LEVEL = 749;
	SKILL_ID_HALLUCINATION_WALKGONO_ASPD_GENSHO = 750;
	SKILL_ID_AUTO_WUG = 751;
	SKILL_ID_CANCEL_EDP_POISON_ATTACK = 752;
	SKILL_ID_SEVERE_RAINSTORM_EX = 753;
	SKILL_ID_COUNT_OF_RG_FOR_BANDING = 754;
	SKILL_ID_DOUBLE_CASTING = 755;
	SKILL_ID_SHIELD_SPELL_REFLECT = 756;
	SKILL_ID_MANA_RECHARGE = 757;
	SKILL_ID_SHIELD_SPELL_LV_1 = 758;
	SKILL_ID_SHIELD_SPELL_LV_2 = 759;
	SKILL_ID_CHIMEITEKINA_KIZU = 760;
	SKILL_ID_ATK_FOR_IRON_NAIL = 761;
	SKILL_ID_HELL_JUDGEMENT = 762;
	SKILL_ID_YAMIKUMO = 763;
	SKILL_ID_MIGITE_TANREN = 764;
	SKILL_ID_HIDARITE_TANREN = 765;
	SKILL_ID_ZYUMONZIGIRI = 766;
	SKILL_ID_YOMIGAESHI = 767;
	SKILL_ID_BAKURETSU_KUNAI = 768;
	SKILL_ID_HAPPO_KUNAI = 769;
	SKILL_ID_FUMASHURIKEN_RANKA = 770;
	SKILL_ID_MAKIBISHI = 771;
	SKILL_ID_MUCHANAGE = 772;
	SKILL_ID_MEIKYO_SHISUI = 773;
	SKILL_ID_GENZYUTSU_KAGEMUSHA = 774;
	SKILL_ID_GENZYUTSU_KYOGAKU = 775;
	SKILL_ID_GENZYUTSU_ZYUSATSU = 776;
	SKILL_ID_GENZYUTSU_GENWAKU = 777;
	SKILL_ID_IZAYOI = 778;
	SKILL_ID_HIFU_ENTEN = 779;
	SKILL_ID_HYOFU_FUBUKI = 780;
	SKILL_ID_FUFU_SEIRAN = 781;
	SKILL_ID_DOFU_GOKAI = 782;
	SKILL_ID_ZYUTSUSHIKI_KAIHO = 783;
	SKILL_ID_ZYUTSUSHIKI_TENKAI = 784;
	SKILL_ID_GENZYUTSU_KAGEFUMI = 785;
	SKILL_ID_GENZYUTSU_KYOMUNOKAGE = 786;
	SKILL_ID_GENZYUTSU_BUNSHIN = 787;
	SKILL_ID_GENZYUTSU_ZANGETSU = 788;
	SKILL_ID_GENZYUTSU_KOUGETSU = 789;
	SKILL_ID_GENZYUTSU_OBOROGENSO = 790;
	SKILL_ID_FU_ELEMENT_OF_FU = 791;
	SKILL_ID_FU_COUNT_OF_FU = 792;
	SKILL_ID_HPSPCONF_FOR_GENZYUTSU_ZANGETSU = 793;
	SKILL_ID_WATER_DRAGON_BREATH = 794;
	SKILL_ID_UNLIMIT = 795;
	SKILL_ID_OFFERTORIUM = 796;
	SKILL_ID_DARK_CRAW = 797;
	SKILL_ID_TELECHINESIS_INSTENCE = 798;
	SKILL_ID_SENKO_RENGEKI = 799;
	SKILL_ID_COMBO_SANDAN_MONK = 800;
	SKILL_ID_COMBO_SANDAN_CHAMP = 801;
	SKILL_ID_COMBO_SORYUKYAKU = 802;
	SKILL_ID_COMBO_RESERVED_803 = 803;
	SKILL_ID_COMBO_RESERVED_804 = 804;
	SKILL_ID_COMBO_RESERVED_805 = 805;
	SKILL_ID_COMBO_RESERVED_806 = 806;
	SKILL_ID_COMBO_RESERVED_807 = 807;
	SKILL_ID_COMBO_RESERVED_808 = 808;
	SKILL_ID_COMBO_RESERVED_809 = 809;
	SKILL_ID_EARTH_QUAKE = 810;
	SKILL_ID_MAGMA_ILLUPTION = 811;
	SKILL_ID_SERE = 812;
	SKILL_ID_SERE_MODE = 813;
	SKILL_ID_SERE_SUPPORT_SKILL = 814;
	SKILL_ID_HOMLV_FOR_PYROCLASTIC = 815;
	SKILL_ID_PYROCLASTIC = 816;
	SKILL_ID_OVERED_BOOST = 817;
	SKILL_ID_GRANITIC_ARMOR = 818;
	SKILL_ID_PAIN_KILLER = 819;
	SKILL_ID_DEFENCE = 820;
	SKILL_ID_ATK_PLUS_AFTER_SENKO_RENGEKI = 821;
	SKILL_ID_RICHS_COIN = 822;
	SKILL_ID_FALLIN_ANGEL = 823;
	SKILL_ID_SHUTTER_STORM = 824;
	SKILL_ID_MASS_SPIRAL = 825;
	SKILL_ID_ETERNAL_CHAIN = 826;
	SKILL_ID_HOWLING_MINE = 827;
	SKILL_ID_FIRE_RAIN = 828;
	SKILL_ID_FRICKER = 829;
	SKILL_ID_FIRE_DANCE = 830;
	SKILL_ID_BUNISHING_BASTER = 831;
	SKILL_ID_UNTIMATERIAL_BLAST = 832;
	SKILL_ID_QUICKDRAW_SHOT = 833;
	SKILL_ID_DRAGON_TAIL = 834;
	SKILL_ID_ROUND_TRIP = 835;
	SKILL_ID_HEAT_BARREL = 836;
	SKILL_ID_HEAT_BARREL_COIN_COUNT = 837;
	SKILL_ID_SLUG_SHOT = 838;
	SKILL_ID_HAMMER_OF_GOD = 839;
	SKILL_ID_CRYMSON_MARKER = 840;
	SKILL_ID_PLATINUM_ALTER = 841;
	SKILL_ID_PLATINUM_ALTER_COIN_COUNT = 842;
	SKILL_ID_BIND_TRAP = 843;
	SKILL_ID_HOWLING_MINE_APPEND = 844;
	SKILL_ID_AS_QUICKDRAW = 845;
	SKILL_ID_KIHON_SKILL = 846;
	SKILL_ID_REDEMPTIO = 847;
	SKILL_ID_SIGHT_BLASTER = 848;
	SKILL_ID_GREED = 849;
	SKILL_ID_FAKE_ZENY = 850;
	SKILL_ID_AUTO_GUARD = 851;
	SKILL_ID_SHRINK = 852;
	SKILL_ID_KIKO_TENI = 853;
	SKILL_ID_CREATE_CONVERTER = 854;
	SKILL_ID_FIRE_ELEMENTAL_CHANGE = 855;
	SKILL_ID_WATER_ELEMENTAL_CHANGE = 856;
	SKILL_ID_WIND_ELEMENTAL_CHANGE = 857;
	SKILL_ID_EARTH_ELEMENTAL_CHANGE = 858;
	SKILL_ID_SEIMEI_RINRI = 859;
	SKILL_ID_ANSOKU = 860;
	SKILL_ID_CALL_HOMUNCULUS = 861;
	SKILL_ID_RESURRECTION_HOMUNCULUS = 862;
	SKILL_ID_GANBANTEIN = 863;
	SKILL_ID_BUKISEIREN = 864;
	SKILL_ID_PRESSURE_MISS = 865;
	SKILL_ID_FULL_STRIP = 866;
	SKILL_ID_PRESERVE = 867;
	SKILL_ID_WATASHIWO_SHIBARANAIDE = 868;
	SKILL_ID_HELLMODENO_TUE = 869;
	SKILL_ID_TSUKIAKARINO_SHITADE = 870;
	SKILL_ID_SEIMEIRYOKU_HENKAN = 871;
	SKILL_ID_SPIDER_WEB = 872;
	SKILL_ID_WALL_OF_FOG = 873;
	SKILL_ID_SLIMPOTION_PITCHER = 874;
	SKILL_ID_FULL_CHEMICAL_CHARGE = 875;
	SKILL_ID_SHOKUBUTSU_SAIBAI = 876;
	SKILL_ID_KNIGHTNO_TAMASHI = 877;
	SKILL_ID_ASSASINNO_TAMASHI = 878;
	SKILL_ID_PRIESTNO_TAMASHI = 879;
	SKILL_ID_HUNTERNO_TAMASHI = 880;
	SKILL_ID_WIZARDNO_TAMASHI = 881;
	SKILL_ID_BLACKSMITHNO_TAMASHI = 882;
	SKILL_ID_CRUSADERNO_TAMASHI = 883;
	SKILL_ID_ROGUENO_TAMASHI = 884;
	SKILL_ID_MONKNO_TAMASHI = 885;
	SKILL_ID_BARDTO_DANCERNO_TAMASHI = 886;
	SKILL_ID_SAGENO_TAMASHI = 887;
	SKILL_ID_ALCHEMISTNO_TAMASHI = 888;
	SKILL_ID_KENSENO_TAMASHI = 889;
	SKILL_ID_SOULLINKERNO_TAMASHI = 890;
	SKILL_ID_FLIP_THE_COIN = 891;
	SKILL_ID_KINGS_GRACE = 892;
	SKILL_ID_ESCAPE = 893;
	SKILL_ID_FRIGNO_UTA = 894;
	SKILL_ID_ELEMENTAL_SHIELD = 895;
	SKILL_ID_ILLUSION_DOOPING = 896;
	SKILL_ID_DARK_CROSS = 897;
	SKILL_ID_INTIMIDATE_FOR_CLONE = 898;
	SKILL_ID_COMBO_GIGANTSET_JOINT_BEAT = 899;
	SKILL_ID_COMBO_GIGANTSET_SPIRAL_PIERCE = 900;
	SKILL_ID_FULLSLOT = 901;
	SKILL_ID_DORAM_KIHON_SKILL = 902;
	SKILL_ID_KAMITSUKU = 903;
	SKILL_ID_KAKURERU = 904;
	SKILL_ID_HIKKAKU = 905;
	SKILL_ID_UZUKUMARU = 906;
	SKILL_ID_NYAN_JAMP = 907;
	SKILL_ID_NYAN_TAMASHI = 908;
	SKILL_ID_SOUL_ATTACK = 909;
	SKILL_ID_SHINSENNA_EBI = 910;
	SKILL_ID_EBI_ZANMAI = 911;
	SKILL_ID_OTORO = 912;
	SKILL_ID_MAGURO_SHIELD = 913;
	SKILL_ID_UMINO_CHIKARA = 914;
	SKILL_ID_SEAFOOD_KEI_SHUTOKU_LEVEL_GOKEI = 915;
	SKILL_ID_GROOMING = 916;
	SKILL_ID_NODOWO_NARASU = 917;
	SKILL_ID_EBI_PARTY = 918;
	SKILL_ID_UMINO_TAMASHI = 919;
	SKILL_ID_MATATABI_LANCE = 920;
	SKILL_ID_MATATABINO_NEKKO = 921;
	SKILL_ID_INUHAKKA_METEOR = 922;
	SKILL_ID_INUHAKKA_SHOWER = 923;
	SKILL_ID_DAICHINO_CHIKARA = 924;
	SKILL_ID_PLANT_KEI_SHUTOKU_LEVEL_GOKEI = 925;
	SKILL_ID_CHATTERING = 926;
	SKILL_ID_MYAUMYAU = 927;
	SKILL_ID_NYAN_GRASS = 928;
	SKILL_ID_DAICHINO_TAMASHI = 929;
	SKILL_ID_PIKKI_TSUKI = 930;
	SKILL_ID_ARCLOUSE_DASH = 931;
	SKILL_ID_TAROUNO_KIZU = 932;
	SKILL_ID_CARROT_BEAT = 933;
	SKILL_ID_SEIMEINO_CHIKARA = 934;
	SKILL_ID_ANIMAL_KEI_SHUTOKU_LEVEL_GOKEI = 935;
	SKILL_ID_KEIKAI = 936;
	SKILL_ID_MURENO_CHIKARA = 937;
	SKILL_ID_SAVAGENO_TAMASHI = 938;
	SKILL_ID_SEIMEINO_TAMASHI = 939;
	SKILL_ID_DAICHINO_TAMASHI_KOKA_MATATABINO_NEKKO = 940;
	SKILL_ID_DAICHINO_TAMASHI_KOKA_INUHAKKA_SHOWER = 941;
	SKILL_ID_DAICHINO_TAMASHI_KOKA_NYAN_GRASS = 942;
	SKILL_ID_SEIMEINO_TAMASHI_KOKA_NOKORI_HP = 943;
	SKILL_ID_BREAK_THROUGH = 944;
	SKILL_ID_TRANSCENDENCE = 945;
	SKILL_ID_TENSHISAMA_TASUKETE = 946;
	SKILL_ID_TAIYOTO_TSUKITO_HOSHINO_KIROKU = 947;
	SKILL_ID_TAIYOTO_TSUKITO_HOSHINO_ZYOKA = 948;
	SKILL_ID_TAIYONO_KAMAE = 949;
	SKILL_ID_KOEN_KYAKU = 950;
	SKILL_ID_TAIYO_BAKUHATSU = 951;
	SKILL_ID_TAIYONO_HIKARI = 952;
	SKILL_ID_TSUKINO_KAMAE = 953;
	SKILL_ID_SAKUGETSU_KYAKU = 954;
	SKILL_ID_MANGETSU_KYAKU = 955;
	SKILL_ID_TSUKINO_HIKARI = 956;
	SKILL_ID_HOSHINO_KAMAE = 957;
	SKILL_ID_SENKO_KYAKU = 958;
	SKILL_ID_RYUSE_RAKKA = 959;
	SKILL_ID_HOSHINO_HIKARI = 960;
	SKILL_ID_UCHUNO_KAMAE = 961;
	SKILL_ID_ZYURYOKU_CHOSE = 962;
	SKILL_ID_SHINSE_BAKUHATSU = 963;
	SKILL_ID_SEITE_KORIN = 964;
	SKILL_ID_SOSENO_SHO = 965;
	SKILL_ID_ZIGENNO_SHO = 966;
	SKILL_ID_ESHA = 967;
	SKILL_ID_ESPA = 968;
	SKILL_ID_ESFU = 969;
	SKILL_ID_KAUTO = 970;
	SKILL_ID_TAMASHINO_CHIKUSEKI = 971;
	SKILL_ID_TAMASHINO_SHUKAKU = 972;
	SKILL_ID_TAMASHINO_ZYUNKAN = 973;
	SKILL_ID_TAMASHINO_RENKETSU = 974;
	SKILL_ID_SOUL_ENERGY_KENKYU = 975;
	SKILL_ID_SHIRYO_HYOI = 976;
	SKILL_ID_SHIRYO_BAKUHATSU = 977;
	SKILL_ID_TAMASHINO_BUNRETSU = 978;
	SKILL_ID_TAKANO_TAMASHI = 979;
	SKILL_ID_YOSENO_TAMASHI = 980;
	SKILL_ID_KAGENO_TAMASHI = 981;
	SKILL_ID_GOLEMNO_TAMASHI = 982;
	SKILL_ID_TAMASHINO_HOKAI = 983;
	SKILL_ID_COUNT_OF_SOUL_ENERGY = 984;
	SKILL_ID_RYUSE_RAKKA_MODE = 985;
	SKILL_ID_PEONY_MAMY = 986;
	SKILL_ID_PISHARI_HERB = 987;
	SKILL_ID_SEKAIZYUNO_HOKORI = 988;
	SKILL_ID_SNOW_FLIP = 989;
	SKILL_ID_SHOZIGENKAIRYO_ZOKA_R = 990;
	SKILL_ID_VAMPIRE_GIFT = 991;
	SKILL_ID_MIWAKUNO_WINK = 992;
	SKILL_ID_SERIES_SEA_FOOD = 993;
	SKILL_ID_SERIES_PLANT = 994;
	SKILL_ID_SERIES_ANIMAL = 995;
	SKILL_ID_STONE_SKIN = 996;
	SKILL_ID_CRITICAL_WOUNDS = 997;
	SKILL_ID_ODINNO_CHIKARA = 998;
	SKILL_ID_TUZYO_KOGEKI_CALC_RIGHT = 999;
	SKILL_ID_TUZYO_KOGEKI_CALC_LEFT = 1000;
	SKILL_ID_TUZYO_KOGEKI_CALC_KATAR_APPEND = 1001;
	SKILL_ID_SERVANT_WEAPON = 1002;
	SKILL_ID_SERVANT_WEAPON_SIGN = 1003;
	SKILL_ID_SERVANT_WEAPON_PHANTOM = 1004;
	SKILL_ID_SERVANT_WEAPON_DEMOLISION = 1005;
	SKILL_ID_CHARGING_PIERCE = 1006;
	SKILL_ID_TWOHAND_DEFENDING = 1007;
	SKILL_ID_HACK_AND_SLASHER = 1008;
	SKILL_ID_DRAGONIC_AURA = 1009;
	SKILL_ID_MADNESS_CRUSHER = 1010;
	SKILL_ID_VIGOR = 1011;
	SKILL_ID_STORM_SLASH = 1012;
	SKILL_ID_DANCING_KNIFE = 1013;
	SKILL_ID_SAVAGE_IMPACT = 1014;
	SKILL_ID_SHADOW_SENSE = 1015;
	SKILL_ID_ETERNAL_SLASH = 1016;
	SKILL_ID_ENCHANTING_SHADOW = 1017;
	SKILL_ID_POTENT_VENOM = 1018;
	SKILL_ID_SHADOW_EXCEED = 1019;
	SKILL_ID_FATAL_SHADOW_CRAW = 1020;
	SKILL_ID_SHADOW_STAB = 1021;
	SKILL_ID_IMPACT_CRATER = 1022;
	SKILL_ID_REPARATIO = 1023;
	SKILL_ID_MEDIA_REBOTUM = 1024;
	SKILL_ID_DONKI_HON_SHUREN = 1025;
	SKILL_ID_ARUGUTUS_VITA = 1026;
	SKILL_ID_ARUGUTUS_TERUM = 1027;
	SKILL_ID_ARBITRIUM = 1028;
	SKILL_ID_PRESENSE_AKYACE = 1029;
	SKILL_ID_FIDOS_ANIMUS = 1030;
	SKILL_ID_EFIRIGO = 1031;
	SKILL_ID_CONPETENTIA = 1032;
	SKILL_ID_NUMATIC_PROCERA = 1033;
	SKILL_ID_DILECTIO_HEAL = 1034;
	SKILL_ID_RERIGIO = 1035;
	SKILL_ID_BENEDICTUM = 1036;
	SKILL_ID_PETITIO = 1037;
	SKILL_ID_PETITIO_LEARNED = 1038;
	SKILL_ID_PHREMEN = 1039;
	SKILL_ID_ADVANCED_TRAP = 1040;
	SKILL_ID_WIND_SIGN = 1041;
	SKILL_ID_SHIZEN_SHINWA = 1042;
	SKILL_ID_HAWK_RUSH = 1043;
	SKILL_ID_HAWK_MASTERY = 1044;
	SKILL_ID_CALAMITY_GALE = 1045;
	SKILL_ID_HAWK_BOOMERANG = 1046;
	SKILL_ID_GALE_STORM = 1047;
	SKILL_ID_DEEP_BLIND_TRAP = 1048;
	SKILL_ID_SOLID_TRAP = 1049;
	SKILL_ID_SWIFT_TRAP = 1050;
	SKILL_ID_CRESSIVE_VOLT = 1051;
	SKILL_ID_FLAME_TRAP = 1052;
	SKILL_ID_DEADLY_PROJECTION = 1053;
	SKILL_ID_DESTRACTIVE_HURRICANE = 1054;
	SKILL_ID_CLIMAX_HURRICANE_STATE = 1055;
	SKILL_ID_RAIN_OF_CRYSTAL = 1056;
	SKILL_ID_MYSTERY_ILLUSION = 1057;
	SKILL_ID_VIOLENT_QUAKE = 1058;
	SKILL_ID_SOUL_VULKUN_STRIKE = 1059;
	SKILL_ID_STRATUM_TREAMER = 1060;
	SKILL_ID_ALL_BLOOM = 1061;
	SKILL_ID_CRYSTAL_IMPACT = 1062;
	SKILL_ID_TORNADE_STORM = 1063;
	SKILL_ID_FLORAL_FLARE_ROAD = 1064;
	SKILL_ID_CLIMAX = 1065;
	SKILL_ID_ASTRAL_STRIKE = 1066;
	SKILL_ID_ROCK_DOWN = 1067;
	SKILL_ID_STORM_CANNON = 1068;
	SKILL_ID_CRYMSON_ARROW = 1069;
	SKILL_ID_FROZEN_SLASH = 1070;
	SKILL_ID_RYOTETUSE_SHUREN = 1071;
	SKILL_ID_AXE_STOMP = 1072;
	SKILL_ID_RUSH_QUAKE = 1073;
	SKILL_ID_RUSH_STATE = 1074;
	SKILL_ID_SOCHI_SEIZO = 1075;
	SKILL_ID_KOGEKI_SOCHI_YUKOKA = 1076;
	SKILL_ID_BOGYO_SOCHI_YUKOKA = 1077;
	SKILL_ID_TWO_AXE_DEFENDING = 1078;
	SKILL_ID_ABR_MASTERY = 1079;
	SKILL_ID_ABR_BATTLE_WARRIER = 1080;
	SKILL_ID_ABR_DUAL_CANNON = 1081;
	SKILL_ID_ABR_MOTHER_NET = 1082;
	SKILL_ID_ABR_INFINITY = 1083;
	SKILL_ID_GUARD_STANCE = 1084;
	SKILL_ID_GUARDIAN_SHIELD = 1085;
	SKILL_ID_REBOUND_SHIELD = 1086;
	SKILL_ID_TATE_SHUREN = 1087;
	SKILL_ID_YARI_KATATE_KEN_SHUREN = 1088;
	SKILL_ID_ATTACK_STANCE = 1089;
	SKILL_ID_ULTIMATE_SACRIFICE = 1090;
	SKILL_ID_HOLY_SHIELD = 1091;
	SKILL_ID_GRAND_JUDGEMENT = 1092;
	SKILL_ID_JUDGEMENT_CROSS = 1093;
	SKILL_ID_SHIELD_SHOOTING = 1094;
	SKILL_ID_OVER_SLASH = 1095;
	SKILL_ID_CROSS_RAIN = 1096;
	SKILL_ID_TANKEN_YUMI_SHUREN = 1097;
	SKILL_ID_MAHOKEN_SHUREN = 1098;
	SKILL_ID_STRIP_SHADOW = 1099;
	SKILL_ID_ABYSS_DAGGER = 1100;
	SKILL_ID_UNLUCKY_RUSH = 1101;
	SKILL_ID_CHAIN_REACTION_SHOT = 1102;
	SKILL_ID_FROM_THE_ABYSS = 1103;
	SKILL_ID_ABYSS_SLAYER = 1104;
	SKILL_ID_OMEGA_ABYSS_STRIKE = 1105;
	SKILL_ID_DEFT_STAB = 1106;
	SKILL_ID_ABYSS_SQUARE = 1107;
	SKILL_ID_ABYSS_SQUARE_LEARNED_LEVEL = 1108;
	SKILL_ID_FLANGE_SHOT = 1109;
	SKILL_ID_KYOZINNA_SHINNEN = 1110;
	SKILL_ID_KENKONA_SHINNEN = 1111;
	SKILL_ID_SHINKONO_ISHI = 1112;
	SKILL_ID_SEYU_SENRE = 1113;
	SKILL_ID_CHUZITSUNA_SHINNEN = 1114;
	SKILL_ID_DAIICHIGEKI_RAKUIN = 1115;
	SKILL_ID_DAIISSHO_SHINNENNO_CHIKARA = 1116;
	SKILL_ID_DAISANGEKI_DANZAI = 1117;
	SKILL_ID_DAISANGEKI_MEKKAGEKI = 1118;
	SKILL_ID_DAISANGEKI_ZYOKA = 1119;
	SKILL_ID_DAINIGEKI_METSUMANO_HI = 1120;
	SKILL_ID_DAINIGEKI_SHINNEN = 1121;
	SKILL_ID_DAINIGEKI_SHINPAN = 1122;
	SKILL_ID_BAKKA_SHINDAN = 1123;
	SKILL_ID_ENKA_METSUMA_SHINDAN = 1124;
	SKILL_ID_DAINISHO_SHIPANSHA = 1125;
	SKILL_ID_SAISHUSHO_METSUMANO_HONO = 1126;
	SKILL_ID_STAGE_MANNER = 1127;
	SKILL_ID_KAISO = 1128;
	SKILL_ID_MYSTIC_SYMPHONY = 1129;
	SKILL_ID_SONATA_OF_KUVASIL = 1130;
	SKILL_ID_ROSE_BLOSSOM = 1131;
	SKILL_ID_RHYTHM_SHOOTING = 1132;
	SKILL_ID_METALIC_FURY = 1133;
	SKILL_ID_SOUND_BLEND = 1134;
	SKILL_ID_GEFFENIA_NOCTURNE = 1135;
	SKILL_ID_LOKINO_KIMAGURE = 1136;
	SKILL_ID_KOINNO_RHAPSODY = 1137;
	SKILL_ID_MUSICAL_INTERLUDE = 1138;
	SKILL_ID_YUYAKENO_SERENADE = 1139;
	SKILL_ID_SHISHATACHIHENO_REQUIEM = 1140;
	SKILL_ID_PRONTERA_MARCH = 1141;
	SKILL_ID_MAHO_HON_SHUREN = 1142;
	SKILL_ID_SPELL_ENCHANTING = 1143;
	SKILL_ID_ACTIVITY_BURN = 1144;
	SKILL_ID_INCREASING_ACTIVITY = 1145;
	SKILL_ID_DIAMOND_STORM = 1146;
	SKILL_ID_LIGHTNING_LAND = 1147;
	SKILL_ID_VENOM_SWAMP = 1148;
	SKILL_ID_CONFLAGRATION = 1149;
	SKILL_ID_TERA_DRIVE = 1150;
	SKILL_ID_ELEMENTAL_SPIRIT_MASTERY = 1151;
	SKILL_ID_SUMMON_ALDOR = 1152;
	SKILL_ID_SUMMON_DILBIO = 1153;
	SKILL_ID_SUMMON_PROCERA = 1154;
	SKILL_ID_SUMMON_TELEMOTUS = 1155;
	SKILL_ID_SUMMON_SERPENSE = 1156;
	SKILL_ID_ELEMENTAL_BASTER = 1157;
	SKILL_ID_ELEMENTAL_VEIL = 1158;
	SKILL_ID_BIONIC_PHARMACY = 1159;
	SKILL_ID_BIONICS_MASTERY = 1160;
	SKILL_ID_HALL_FULL_CHEMICAL_CHARGE = 1161;
	SKILL_ID_FULL_SHADOW_CHARGE = 1162;
	SKILL_ID_ACIDIFIED_ZONE_MIZU = 1163;
	SKILL_ID_ACIDIFIED_ZONE_CHI = 1164;
	SKILL_ID_ACIDIFIED_ZONE_HI = 1165;
	SKILL_ID_ACIDIFIED_ZONE_KAZE = 1166;
	SKILL_ID_CREATE_WOODEN_WARRIER = 1167;
	SKILL_ID_CREATE_WOODEN_FAIRY = 1168;
	SKILL_ID_CREATE_CREAPER = 1169;
	SKILL_ID_RESEARCH_REPORT = 1170;
	SKILL_ID_CREATE_HELL_TREE = 1171;
	SKILL_ID_DRAGONIC_AURA_STATE = 1172;
	SKILL_ID_TENKI_SHUREN = 1173;
	SKILL_ID_HYOHO_SHUREN = 1174;
	SKILL_ID_TENCHI_ICHIYO = 1175;
	SKILL_ID_TAITEN_ICHIYO = 1176;
	SKILL_ID_TENYO = 1177;
	SKILL_ID_TENCHI_ICHIGETSU = 1178;
	SKILL_ID_TAITEN_ICHIGETSU = 1179;
	SKILL_ID_TENGETSU = 1180;
	SKILL_ID_TENCHI_BANSE = 1181;
	SKILL_ID_TENME_RAKUSE = 1182;
	SKILL_ID_TENSE = 1183;
	SKILL_ID_TENRA_BANSHO = 1184;
	SKILL_ID_TENKINO_MI = 1185;
	SKILL_ID_UNKONO_ZYOTAI = 1186;
	SKILL_ID_GOFU_SHUREN = 1187;
	SKILL_ID_REIDOZYUTSU_SHUREN = 1188;
	SKILL_ID_SHUGO_FU = 1189;
	SKILL_ID_BUSHI_FU = 1190;
	SKILL_ID_HOSHI_FU = 1191;
	SKILL_ID_GOKON_ISSHIN = 1192;
	SKILL_ID_ZYOKODO = 1193;
	SKILL_ID_GOGYO_FU = 1194;
	SKILL_ID_REIDO_FU = 1195;
	SKILL_ID_SHIRYO_ZYOKA = 1196;
	SKILL_ID_SEIRYU_FU = 1197;
	SKILL_ID_BYAKKO_FU = 1198;
	SKILL_ID_SUZAKU_FU = 1199;
	SKILL_ID_GENBU_FU = 1200;
	SKILL_ID_SHIHOZIN_FU = 1201;
	SKILL_ID_SHIHO_GOGYO_ZIN = 1202;
	SKILL_ID_TENCHI_SHINRE = 1203;
	SKILL_ID_SHIHO_FU_ZYOTAI = 1204;
	SKILL_ID_SPIRIT_MASTERY = 1205;
	SKILL_ID_SANREI_ITTAI = 1206;
	SKILL_ID_NYAN_BRESSING = 1207;
	SKILL_ID_MARIN_FESTIVAL = 1208;
	SKILL_ID_SAND_FESTIVAL = 1209;
	SKILL_ID_DOKUGAKU_SENTOGAKU = 1210;
	SKILL_ID_DOKUGAKU_MADOGAKU = 1211;
	SKILL_ID_PFI = 1212;
	SKILL_ID_GRENADE_MASTERY = 1213;
	SKILL_ID_INTENSIVE_AIM = 1214;
	SKILL_ID_HIDDEN_CARD = 1215;
	SKILL_ID_ONLY_ONE_BULLET = 1216;
	SKILL_ID_SPIRAL_SHOOTING = 1217;
	SKILL_ID_MAGAZIN_FOR_ONE = 1218;
	SKILL_ID_VIGILANT_AT_NIGHT = 1219;
	SKILL_ID_WILD_FIRE = 1220;
	SKILL_ID_TIGER_SLASH = 1221;
	SKILL_ID_TIGER_HOWLING = 1222;
	SKILL_ID_TIGER_STRIKE = 1223;
	SKILL_ID_NYANTOMO_TEKKO = 1224;
	SKILL_ID_KAGE_NO_MAI = 1225;
	SKILL_ID_KAGE_ISSEN = 1226;
	SKILL_ID_KAGE_GARI = 1227;
	SKILL_ID_GENJUTSU_KAGE_NUI = 1228;
	SKILL_ID_FUMASHURIKEN_SHOUAKU = 1229;
	SKILL_ID_FUMASHURIKEN_KOUCHIKU = 1230;
	SKILL_ID_JUPITER_THUNDER_STORM = 1231;
	SKILL_ID_HELLS_DRIVE = 1232;
	SKILL_ID_NAPALM_VULKAN_STRIKE = 1233;
	SKILL_ID_METEOR_STORM_BUSTER = 1234;
	SKILL_ID_DOUBLE_BOWLING_BASH = 1235;
	SKILL_ID_MEGA_SONIC_BLOW = 1236;
	SKILL_ID_SPARK_BLASTER = 1237;
	SKILL_ID_TRIPLE_LASER = 1238;
	SKILL_ID_EXPLOSIVE_POWDER = 1239;
	SKILL_ID_MEYHEMIC_THORNS = 1240;
	SKILL_ID_DRAGONIC_BREATH = 1241;
	SKILL_ID_MIGHTY_SMASH = 1242;
	SKILL_ID_AXE_STOMP_STATUS = 1243;
	SKILL_ID_SHIELD_SHOOTING_STATE = 1244;
	SKILL_ID_GRAND_JUDGEMENT_STATE = 1245;
	SKILL_ID_NYANTOMO_KENROKU = 1246;
	SKILL_ID_DEER_CANON = 1247;
	SKILL_ID_DEER_BREEZE = 1248;
	SKILL_ID_BASIC_GRENADE = 1249;
	SKILL_ID_HASTY_FIRE_IN_THE_HOLE = 1250;
	SKILL_ID_GRENADES_DROPPING = 1251;
	SKILL_ID_MISSION_BOMBARD = 1252;
	SKILL_ID_SEKIEN_HOU = 1253;
	SKILL_ID_REIKETSU_HOU = 1254;
	SKILL_ID_RAIDEN_HOU = 1255;
	SKILL_ID_KINNRYUU_HOU = 1256;
	SKILL_ID_ANTEN_HOU = 1257;
	SKILL_ID_GENZYUTSU_ANKOKURYUU = 1258;
	SKILL_ID_ANTEN_HOU_LEARNED_LEVEL = 1259;
	SKILL_ID_TAIYOTO_TSUKITO_HOSHINO_HI = 1260;	// 今日の日付（太陽の日、月の日、星の日の判定用）
	SKILL_ID_KAGEMOGURI = 1261;
	SKILL_ID_KAGETOKI = 1262;
	SKILL_ID_KUNAI_WAIKYOKU = 1263;
	SKILL_ID_KUNAI_KAITEN = 1264;
	SKILL_ID_KUNAI_KUSSETSU = 1265;
	SKILL_ID_GENJUTSU_KUNAI = 1266;
	SKILL_ID_SHIELD_CHAIN_RUSH = 1267;
	SKILL_ID_SPIRAL_PIERCE_MAX = 1268;
	SKILL_ID_JACK_FROST_NOVA = 1269;
	SKILL_ID_GROUND_GRAVITATION = 1270;
	SKILL_ID_BREAKING_LIMIT_STATE = 1271;
	SKILL_ID_RULE_BREAK_STATE = 1272;
	SKILL_ID_ABYSS_DAGGER_STATE = 1273;
*/

	window.SkillObjNew = [
		[0,1,"通常攻撃"],
		[1,1,"応急手当","NV_FIRSTAID"],
		[2,1,"死んだふり"],
		[3,10,"剣修練","SM_SWORD"],
		[4,10,"両手剣修練","SM_TWOHAND"],
		[5,10,"HP回復力向上","SM_RECOVERY"],
		[6,10,"バッシュ","SM_BASH"],
		[7,10,"マグナムブレイク","SM_MAGNUM"],
		[8,10,"プロボック","SM_PROVOKE"],
		[9,10,"インデュア", "SM_ENDURE"],
		[10,1,"移動時HP回復","SM_MOVINGRECOVERY"],
		[11,1,"急所攻撃","SM_FATALBLOW"],
		[12,1,"オートバーサーク","SM_AUTOBERSERK"],
		[13,10,"ダブルアタック","TF_DOUBLE"],
		[14,10,"回避率増加","TF_MISS"],
		[15,10,"スティール","TF_STEAL"],
		[16,10,"ハイディング","TF_HIDING"],
		[17,10,"インベナム","TF_POISON"],
		[18,1,"解毒","TF_DETOXIFY"],
		[19,1,"砂まき","TF_SPRINKLESAND"],
		[20,1,"バックステップ","TF_BACKSLIDING"],
		[21,1,"石拾い","TF_PICKSTONE"],
		[22,1,"石投げ","TF_THROWSTONE"],
		[23,10,"ディバインプロテクション","AL_DP"],
		[24,10,"デーモンベイン","AL_DEMONBANE"],
		[25,10,"ヒール","AL_HEAL"],
		[26,1,"キュアー","AL_CURE"],
		[27,10,"速度増加","AL_INCAGI"],
		[28,10,"速度減少","AL_DECAGI"],
		[29,10,"シグナムクルシス","AL_CRUCIS"],
		[30,10,"エンジェラス","AL_ANGELUS"],
		[31,10,"ブレッシング","AL_BLESSING"],
		[32,1,"ニューマ","AL_PNEUMA"],
		[33,1,"アクアベネディクタ","AL_HOLYWATER"],
		[34,1,"ルアフ","AL_RUWACH"],
		[35,2,"テレポート","AL_TELEPORT"],
		[36,4,"ワープポータル","AL_WARP"],
		[37,1,"ホーリーライト","AL_HOLYLIGHT"],
		[38,10,"ふくろうの目","AC_OWL"],
		[39,10,"ワシの目","AC_VULTURE"],
		[40,10,"ダブルストレイフィング","AC_DOUBLE"],
		[41,10,"アローシャワー","AC_SHOWER"],
		[42,10,"集中力向上","AC_CONCENTRATION"],
		[43,1,"矢作成","AC_MAKINGARROW"],
		[44,1,"チャージアロー","AC_CHARGEARROW"],
		[45,10,"SP回復力向上","MG_SRECOVERY"],
		[46,10,"ナパームビート","MG_NAPALMBEAT"],
		[47,10,"ソウルストライク","MG_SOULSTRIKE"],
		[48,10,"セイフティウォール","MG_SAFETYWALL"],
		[49,10,"ストーンカース","MG_STONECURSE"],
		[50,1,"サイト","MG_SIGHT"],
		[51,10,"ファイアーボルト","MG_FIREBOLT"],
		[52,10,"ファイアーボール","MG_FIREBALL"],
		[53,10,"ファイアーウォール","MG_FIREWALL"],
		[54,10,"コールドボルト","MG_COLDBOLT"],
		[55,10,"フロストダイバー","MG_FROSTDIVER"],
		[56,10,"ライトニングボルト","MG_LIGHTNINGBOLT"],
		[57,10,"サンダーストーム","MG_THUNDERSTORM"],
		[58,1,"エナジーコート","MG_ENERGYCOAT"],
		[59,10,"所持限界量増加","MC_INCCARRY"],
		[60,10,"ディスカウント","MC_DISCOUNT"],
		[61,10,"オーバーチャージ","MC_OVERCHARGE"],
		[62,10,"プッシュカート","MC_PUSHCART"],
		[63,1,"アイテム鑑定","MC_IDENTIFY"],
		[64,10,"露店開設","MC_VENDING"],
		[65,10,"メマーナイト","MC_MAMMONITE"],
		[66,1,"カートレボリューション","MC_CARTREVOLUTION"],
		[67,1,"チェンジカート","MC_CHANGECART"],
		[68,1,"ラウドボイス","MC_LOUD"],
		[69,10,"槍修練","KN_SPEARMASTERY"],
		[70,10,"ピアース","KN_PIERCE"],
		[71,10,"スピアスタブ","KN_SPEARSTAB"],
		[72,5,"スピアブーメラン","KN_SPEARBOOMERANG"],
		[73,10,"ブランディッシュスピア","KN_BRANDISHSPEAR"],
		[74,10,"ツーハンドクイッケン","KN_TWOHANDQUICKEN"],
		[75,5,"オートカウンター","KN_AUTOCOUNTER"],
		[76,10,"ボウリングバッシュ","KN_BOWLINGBASH"],
		[77,1,"ライディング","KN_RIDING"],
		[78,5,"騎兵修練","KN_CAVALIERMASTERY"],
		[79,5,"右手修練","AS_RIGHT"],
		[80,5,"左手修練","AS_LEFT"],
		[81,10,"カタール修練","AS_KATAR"],
		[82,10,"クローキング","AS_CLOAKING"],
		[83,10,"ソニックブロー","AS_SONICBLOW"],
		[84,5,"グリムトゥース","AS_GRIMTOOTH"],
		[85,10,"エンチャントポイズン","AS_ENCHANTPOISON"],
		[86,10,"ポイズンリアクト(反撃)","AS_POISONREACT"],
		[87,10,"ベナムダスト","AS_VENOMDUST"],
		[88,10,"ベナムスプラッシャー","AS_SPLASHER"],
		[89,10,"メイス修練","PR_MACEMASTERY"],
		[90,5,"イムポシティオマヌス","PR_IMPOSITIO"],
		[91,3,"サフラギウム","PR_SUFFRAGIUM"],
		[92,5,"アスペルシオ","PR_ASPERSIO"],
		[93,5,"聖体降福","PR_BENEDICTIO"],
		[94,10,"サンクチュアリ","PR_SANCTUARY"],
		[95,1,"リカバリー","PR_STRECOVERY"],
		[96,4,"スローポイズン","PR_SLOWPOISON"],
		[97,4,"(×)リザレクション","ALL_RESURRECTION"],
		[98,10,"キリエエレイソン","PR_KYRIE"],
		[99,5,"マグニフィカート","PR_MAGNIFICAT"],
		[100,5,"グロリア","PR_GLORIA"],
		[101,10,"レックスディビーナ","PR_LEXDIVINA"],
		[102,10,"(×)ターンアンデッド","PR_TURNUNDEAD"],
		[103,1,"レックスエーテルナ","PR_LEXAETERNA"],
		[104,10,"マグヌスエクソシズム","PR_MAGNUS"],
		[105,5,"スキッドトラップ","HT_SKIDTRAP"],
		[106,5,"(仮)ランドマイン","HT_LANDMINE"],
		[107,5,"アンクルスネア","HT_ANKLESNARE"],
		[108,5,"フラッシャー","HT_FLASHER"],
		[109,5,"ショックウェーブトラップ","HT_SHOCKWAVE"],
		[110,5,"サンドマン","HT_SANDMAN"],
		[111,5,"フリージングトラップ","HT_FREEZINGTRAP"],
		[112,5,"(？)ブラストマイン","HT_BLASTMINE"],
		[113,5,"(？)クレイモアトラップ","HT_CLAYMORETRAP"],
		[114,1,"リムーブトラップ","HT_REMOVETRAP"],
		[115,1,"トーキーボックス","HT_TALKIEBOX"],
		[116,10,"ビーストベイン","HT_BEASTBANE"],
		[117,1,"ファルコンマスタリー","HT_FALCON"],
		[118,5,"ブリッツビート","HT_BLITZBEAT"],
		[119,10,"スチールクロウ","HT_STEELCROW"],
		[120,4,"ディテクティング","HT_DETECTING"],
		[121,5,"スプリングトラップ","HT_SPRINGTRAP"],
		[122,10,"ファイアーピラー","WZ_FIREPILLAR"],
		[123,1,"モンスター情報","WZ_ESTIMATION"],
		[124,10,"サイトラッシャー","WZ_SIGHTRASHER"],
		[125,10,"メテオストーム","WZ_METEOR"],
		[126,10,"ユピテルサンダー","WZ_JUPITEL"],
		[127,10,"ロードオブヴァーミリオン","WZ_VERMILION"],
		[128,5,"ウォーターボール","WZ_WATERBALL"],
		[129,10,"アイスウォール","WZ_ICEWALL"],
		[130,10,"フロストノヴァ","WZ_FROSTNOVA"],
		[131,10,"ストームガスト","WZ_STORMGUST"],
		[132,5,"アーススパイク","WZ_EARTHSPIKE"],
		[133,5,"ヘヴンズドライブ","WZ_HEAVENDRIVE"],
		[134,5,"クァグマイア","WZ_QUAGMIRE"],
		[135,5,"鉄製造","BS_IRON"],
		[136,5,"鋼鉄製造","BS_STEEL"],
		[137,5,"属性石製造","BS_ENCHANTEDSTONE"],
		[138,5,"オリデオコン研究","BS_ORIDEOCON"],
		[139,3,"短剣製作","BS_DAGGER"],
		[140,3,"剣製作","BS_SWORD"],
		[141,3,"両手剣製作","BS_TWOHANDSWORD"],
		[142,3,"斧製作","BS_AXE"],
		[143,3,"メイス製作","BS_MACE"],
		[144,3,"ナックル製作","BS_KNUCKLE"],
		[145,3,"槍製作","BS_SPEAR"],
		[146,1,"ヒルトバインディング","BS_HILTBINDING"],
		[147,1,"鉱石発見","BS_FINDINGORE"],
		[148,10,"武器研究","BS_WEAPONRESEARCH"],
		[149,1,"武器修理","BS_REPAIRWEAPON"],
		[150,5,"スキンテンパリング","BS_SKINTEMPER"],
		[151,5,"ハンマーフォール","BS_HAMMERFALL"],
		[152,5,"アドレナリンラッシュ","BS_ADRENALINE"],
		[153,5,"ウェポンパーフェクション","BS_WEAPONPERFECT"],
		[154,5,"オーバートラスト","BS_OVERTHRUST"],
		[155,5,"マキシマイズパワー","BS_MAXIMIZE"],
		[156,10,"フェイス","CR_TRUST"],
		[157,10,"オートガード","CR_AUTOGUARD"],
		[158,5,"シールドチャージ","CR_SHIELDCHARGE"],
		[159,5,"シールドブーメラン","CR_SHIELDBOOMERANG"],
		[160,10,"リフレクトシールド","CR_REFLECTSHIELD"],
		[161,10,"ホーリークロス","CR_HOLYCROSS"],
		[162,10,"グランドクロス","CR_GRANDCROSS"],
		[163,5,"ディボーション","CR_DEVOTION"],
		[164,5,"プロヴィデンス","CR_PROVIDENCE"],
		[165,5,"ディフェンダー","CR_DEFENDER"],
		[166,10,"スピアクイッケン","CR_SPEARQUICKEN"],
		[167,10,"スナッチャー","RG_SNATCHER"],
		[168,10,"スティールコイン","RG_STEALCOIN"],
		[169,10,"バックスタブ","RG_BACKSTAP"],
		[170,5,"トンネルドライブ","RG_TUNNELDRIVE"],
		[171,5,"サプライズアタック","RG_RAID"],
		[172,5,"ストリップウェポン","RG_STRIPWEAPON"],
		[173,5,"ストリップシールド","RG_STRIPSHIELD"],
		[174,5,"ストリップアーマー","RG_STRIPARMOR"],
		[175,5,"ストリップヘルム","RG_STRIPHELM"],
		[176,5,"インティミデイト","RG_INTIMIDATE"],
		[177,1,"グラフィティ","RG_GRAFFITI"],
		[178,5,"フラッググラフィティ","RG_FLAGGRAFFITI"],
		[179,1,"クリーナー","RG_CLEANER"],
		[180,1,"ギャングスターパラダイス","RG_GANGSTER"],
		[181,5,"コンパルションディスカウント","RG_COMPULSION"],
		[182,10,"クローンスキル","RG_PLAGIARISM"],
		[183,10,"鉄拳","MO_IRONHAND"],
		[184,5,"息吹","MO_SPIRITSRECOVERY"],
		[185,5,"気功(気弾数)","MO_CALLSPIRITS"],
		[186,1,"気奪","MO_ABSORBSPIRITS"],
		[187,10,"三段掌","MO_TRIPLEATTACK"],
		[188,5,"連打掌","MO_CHAINCOMBO"],
		[189,5,"猛龍拳","MO_COMBOFINISH"],
		[190,1,"残影","MO_BODYRELOCATION"],
		[191,10,"見切り","MO_DODGE"],
		[192,5,"指弾(Hit数=気功)","MO_FINGEROFFENSIVE"],
		[193,5,"発勁","MO_INVESTIGATE"],
		[194,5,"白刃取り","MO_BLADESTOP"],
		[195,5,"爆裂波動","MO_EXPLOSIONSPIRITS"],
		[196,5,"金剛","MO_STEELBODY"],
		[197,5,"阿修羅覇凰拳(SP調整可)","MO_EXTREMITYFIST"],
		[198,10,"楽器の練習","BA_MUSICALLESSON"],
		[199,5,"ミュージカルストライク","BA_MUSICALSTRIKE"],
		[200,5,"不協和音","BA_DISSONANCE"],
		[201,5,"寒いジョーク","BA_FROSTJOKE"],
		[202,10,"口笛","BA_WHISTLE"],
		[203,10,"夕陽のアサシンクロス","BA_ASSASSINCROSS"],
		[204,10,"ブラギの詩","BA_POEMBRAGI"],
		[205,10,"イドゥンの林檎","BA_APPLEIDUN"],
		[206,10,"ダンスの練習","DC_DANCINGLESSON"],
		[207,5,"矢撃ち","DC_THROWARROW"],
		[208,5,"自分勝手なダンス","DC_UGLYDANCE"],
		[209,5,"スクリーム","DC_SCREAM"],
		[210,10,"ハミング","DC_HUMMING"],
		[211,10,"私を忘れないで…","DC_DONTFORGETME"],
		[212,10,"幸運のキス","DC_FORTUNEKISS"],
		[213,10,"サービスフォーユー","DC_SERVICEFORYOU"],
		[214,1,"アドリブ","BD_ADAPTATION"],
		[215,1,"アンコール","BD_ENCORE"],
		[216,1,"子守歌","BD_LULLABY"],
		[217,5,"ニヨルドの宴","BD_RICHMANKIM"],
		[218,1,"永遠の混沌","BD_ETERNALCHAOS"],
		[219,5,"戦太鼓の響き","BD_DRUMBATTLEFIELD"],
		[220,5,"ニーベルングの指輪","BD_RINGNIBELUNGEN"],
		[221,1,"ロキの叫び","BD_ROKISWEIL"],
		[222,1,"深淵の中に","BD_INTOABYSS"],
		[223,5,"不死身のジークフリード","BD_SIEGFRIED"],
		[224,10,"アドバンスドブック","SA_ADVANCEDBOOK"],
		[225,5,"キャストキャンセル","SA_CASTCANCEL"],
		[226,5,"マジックロッド","SA_MAGICROD"],
		[227,5,"スペルブレイカー","SA_SPELLBREAKER"],
		[228,10,"フリーキャスト","SA_FREECAST"],
		[229,10,"(仮)オートマジシャンスペル","SA_AUTOSPELL"],
		[230,5,"フレイムランチャー","SA_FLAMELAUNCHER"],
		[231,5,"フロストウェポン","SA_FROSTWEAPON"],
		[232,5,"ライトニングローダー","SA_LIGHTNINGLOADER"],
		[233,5,"サイズミックウェポン","SA_SEISMICWEAPON"],
		[234,5,"ドラゴノロジー","SA_DRAGONOLOGY"],
		[235,5,"ボルケーノ","SA_VOLCANO"],
		[236,5,"デリュージ","SA_DELUGE"],
		[237,5,"バイオレントゲイル","SA_VIOLENTGALE"],
		[238,5,"ランドプロテクター","SA_LANDPROTECTOR"],
		[239,5,"ディスペル","SA_DISPELL"],
		[240,10,"アブラカタブラ","SA_ABRACADABRA"],
		[241,10,"斧修練","AM_AXEMASTERY"],
		[242,10,"ラーニングポーション","AM_LEARNINGPOTION"],
		[243,10,"ファーマシー","AM_PHARMACY"],
		[244,5,"アシッドテラー","AM_ACIDTERROR"],
		[245,5,"ポーションピッチャー","AM_POTIONPITCHER"],
		[246,5,"バイオプラント","AM_CANNIBALIZE"],
		[247,5,"スフィアーマイン","AM_SPHEREMINE"],
		[248,5,"デモンストレーション","AM_DEMONSTRATION"],
		[249,5,"ケミカルウェポンチャージ","AM_CP_WEAPON"],
		[250,5,"ケミカルシールドチャージ","AM_CP_SHIELD"],
		[251,5,"ケミカルアーマーチャージ","AM_CP_ARMOR"],
		[252,5,"ケミカルヘルムチャージ","AM_CP_HELM"],
		[253,1,"爆裂波動(Sノビ)"],
		[254,5,"オーラブレイド","LK_AURABLADE"],
		[255,10,"パリイング","LK_PARRYING"],
		[256,5,"コンセントレイション","LK_CONCENTRATION"],
		[257,1,"テンションリラックス","LK_TENSIONRELAX"],
		[258,1,"バーサーク","LK_BERSERK"],
		[259,5,"スパイラルピアース","LK_SPIRALPIERCE"],
		[260,5,"ヘッドクラッシュ","LK_HEADCRUSH"],
		[261,10,"ジョイントビート","LK_JOINTBEAT"],
		[262,5,"カタール研究","ASC_KATAR"],
		[263,10,"ソウルブレイカー","ASC_BREAKER"],
		[264,10,"メテオアサルト","ASC_METEORASSAULT"],
		[265,1,"クリエイトデッドリーポイズン","ASC_CDP"],
		[266,5,"(仮)エンチャントデッドリーポイズン","ASC_EDP"],
		[267,5,"アスムプティオ","HP_ASSUMPTIO"],
		[268,5,"バジリカ","HP_BASILICA"],
		[269,10,"メディタティオ","HP_MEDITATIO"],
		[270,10,"トゥルーサイト","SN_SIGHT"],
		[271,5,"ファルコンアサルト","SN_FALCONASSAULT"],
		[272,5,"シャープシューティング","SN_SHARPSHOOTING"],
		[273,10,"ウィンドウォーク","SN_WINDWALK"],
		[274,10,"ソウルドレイン","HW_SOULDRAIN"],
		[275,1,"マジッククラッシャー","HW_MAGICCRASHER"],
		[276,10,"魔法力増幅","HW_MAGICPOWER"],
		[277,5,"ナパームバルカン","HW_NAPALMVULCAN"],
		[278,10,"メルトダウン","WS_MELTDOWN"],
		[279,3,"お金製造"],
		[280,3,"塊製造"],
		[281,1,"カートブースト","WS_CARTBOOST"],
		[282,5,"運命のタロットカード","CG_TAROTCARD"],
		[283,5,"プレッシャー","PA_PRESSURE"],
		[284,5,"サクリファイス","PA_SACRIFICE"],
		[285,10,"ゴスペル","PA_GOSPEL"],
		[286,5,"チェイスウォーク(STR+)","ST_CHASEWALK"],
		[287,5,"リジェクトソード","ST_REJECTSWORD"],
		[288,5,"猛虎硬爬山","CH_PALMSTRIKE"],
		[289,5,"伏虎拳","CH_TIGERFIST"],
		[290,10,"連柱崩撃","CH_CHAINCRUSH"],
		[291,1,"ソウルコレクト"],
		[292,10,"アローバルカン","CG_ARROWVULCAN"],
		[293,1,"練気功","CH_SOULCOLLECT"],
		[294,1,"マリオネットコントロール","CG_MARIONETTE"],
		[295,5,"ライフコンバージョン"],
		[296,1,"ソウルチェンジ","PF_SOULCHANGE"],
		[297,5,"ソウルバーン","PF_SOULBURN"],
		[298,5,"マインドブレイカー","PF_MINDBREAKER"],
		[299,5,"アルケミー"],
		[300,5,"ポーションシノプス"],
		[301,1,"連打掌修得時の三段掌ディレイ増加"],
		[302,1,"トマホーク投げ"],
		[303,5,"パルスストライク"],
		[304,1,"バーサークピッチャー","AM_BERSERKPITCHER"],
		[305,7,"ティオアプチャギ(ダッシュ中)"],
		[306,1,"ベナムナイフ","AS_VENOMKNIFE"],
		[307,1,"ファンタズミックアロー","HT_PHANTASMIC"],
		[308,1,"チャージアタック"],
		[309,1,"無死亡ボーナス"],
		[310,1,"結婚ステータス-1付与"],
		[311,7,"製作スキルマスター数(達人の斧用)"],
		[312,5,"ダークストライク"],
		[313,1,""],
		[314,1,""],
		[315,1,""],
		[316,1,""],
		[317,1,"温もり"],
		[318,1,"温もり(壁押付)"],
		[319,10,"ヘヴンズドライブ(盗作用Ex)"],
		[320,10,"ウォーターボール(盗作用Ex)"],
		[321,5,"阿修羅覇凰拳(MaxSP-1固定)"],
		[322,1,"メモライズ(5回制限未計算)","PF_MEMORIZE"],
		[323,1,"(現在この欄は未使用)"],
		[324,5,"(△)シールドチェーン","PA_SHIELDCHAIN"],
		[325,5,"グラビテーションフィールド","HW_GRAVITATION"],
		[326,10,"カートターミネーション","WS_CARTTERMINATION"],
		[327,5,"オーバートラストマックス","WS_OVERTHRUSTMAX"],
		[328,10,"(仮)アシッドデモンストレーション","CR_ACIDDEMONSTRATION"],
		[329,10,"タイリギ(蹴威力UP)","TK_RUN"],
		[330,1,"フェオリチャギの構え","TK_READYSTORM"],
		[331,7,"フェオリチャギ","TK_STORMKICK"],
		[332,1,"ネリョチャギの構え","TK_READYDOWN"],
		[333,7,"ネリョチャギ","TK_DOWNKICK"],
		[334,1,"トルリョチャギの構え","TK_READYTURN"],
		[335,7,"トルリョチャギ","TK_TURNKICK"],
		[336,1,"アプチャオルリギの構え","TK_READYCOUNTER"],
		[337,7,"アプチャオルリギ","TK_COUNTER"],
		[338,1,"落法(調整中)","TK_DODGE"],
		[339,7,"ティオアプチャギ","TK_JUMPKICK"],
		[340,10,"穏やかな休息","TK_HPTIME"],
		[341,10,"楽しい休息","TK_SPTIME"],
		[342,5,"ファイト","TK_POWER"],
		[343,5,"ノピティギ","TK_HIGHJUMP"],
		[344,1,"テコンミッション","TK_MISSION"],
		[345,1,"テコンランカー状態"],
		[346,7,"暖かい風","TK_SEVENWIND"],
		[347,3,"太陽と月と星の感情","SG_FEEL"],
		[348,3,"太陽の温もり","SG_SUN_WARM"],
		[349,3,"月の温もり","SG_MOON_WARM"],
		[350,3,"星の温もり","SG_STAR_WARM"],
		[351,3,"太陽と月と星の憎しみ","SG_HATE"],
		[352,3,"太陽の怒り","SG_SUN_ANGER"],
		[353,3,"月の怒り","SG_MOON_ANGER"],
		[354,3,"星の怒り","SG_STAR_ANGER"],
		[355,4,"太陽の安楽","SG_SUN_COMFORT"],
		[356,4,"月の安楽","SG_MOON_COMFORT"],
		[357,4,"星の安楽","SG_STAR_COMFORT"],
		[358,5,"太陽の祝福","SG_SUN_BLESS"],
		[359,5,"月の祝福","SG_MOON_BLESS"],
		[360,5,"星の祝福","SG_STAR_BLESS"],
		[361,10,"太陽と月と星の悪魔","SG_DEVIL"],
		[362,3,"太陽と月と星の友","SG_FRIEND"],
		[363,10,"太陽と月と星の知識","SG_KNOWLEDGE"],
		[364,1,"太陽と月と星の融合","SG_FUSION"],
		[365,1,"太陽と月と星の奇跡"],
		[366,1,"太陽と月と星の天使"],
		[367,5,"～の祝福(経験値増加率)"],
		[368,7,"カイゼル","SL_KAIZEL"],
		[369,7,"カアヒ","SL_KAAHI"],
		[370,3,"カウプ","SL_KAUPE"],
		[371,7,"カイト","SL_KAITE"],
		[372,7,"カイナ","SL_KAINA"],
		[373,7,"エスティン","SL_STIN"],
		[374,7,"エストン","SL_STUN"],
		[375,10,"エスマ","SL_SMA"],
		[376,7,"エスウ","SL_SWOO"],
		[377,3,"エスカ","SL_SKA"],
		[378,3,"エスク","SL_SKE"],
		[379,1,"タイリギスパート状態(STR+状態)"],
		[380,11,"自分以外のPT人数(ファイト用)"],
		[381,1,"ソニックアクセラレーション","AS_SONICACCEL"],
		[382,1,"寸勁","MO_BALKYOUNG"],
		[383,1,"クローズコンファイン","RG_CLOSECONFINE"],
		[384,5,"シールドブーメラン(SL魂版)"],
		[385,1,"スーパーノービスの魂","SL_SUPERNOVICE"],
		[386,1,"ワンハンドクイッケン(SL魂)"],
		[387,1,"ホーリーライト(SL魂版)"],
		[388,10,"ソニックブロー(SL魂版)"],
		[389,1,"フルアドレナリンラッシュ","BS_ADRENALINE2"],
		[390,1,"ハンターの魂"],
		[391,1,"ビーストストレイフィング","HT_POWER"],
		[392,1,"転生一次職の魂","SL_HIGH"],
		[393,10,"投擲修練","NJ_TOBIDOUGU"],
		[394,10,"(△)手裏剣投げ","NJ_SYURIKEN"],
		[395,5,"(△)苦無投げ","NJ_KUNAI"],
		[396,5,"(△)風魔手裏剣投げ","NJ_HUUMA"],
		[397,10,"銭投げ","NJ_ZENYNAGE"],
		[398,5,"畳返し","NJ_TATAMIGAESHI"],
		[399,5,"影跳び","NJ_SHADOWJUMP"],
		[400,10,"(△)霞斬り","NJ_KASUMIKIRI"],
		[401,5,"(△)影斬り","NJ_KIRIKAGE"],
		[402,5,"空蝉","NJ_UTSUSEMI"],
		[403,10,"影分身","NJ_BUNSINJYUTSU"],
		[404,5,"念","NJ_NEN"],
		[405,10,"一閃","NJ_ISSEN"],
		[406,10,"忍法修練","NJ_NINPOU"],
		[407,10,"紅炎華","NJ_KOUENKA"],
		[408,10,"火炎陣","NJ_KAENSIN"],
		[409,5,"龍炎陣","NJ_BAKUENRYU"],
		[410,10,"氷閃槍","NJ_HYOUSENSOU"],
		[411,10,"水遁","NJ_SUITON"],
		[412,5,"(△)氷柱落し","NJ_HYOUSYOURAKU"],
		[413,10,"風刃","NJ_HUUJIN"],
		[414,5,"(△)雷撃砕","NJ_RAIGEKISAI"],
		[415,5,"(△)朔風","NJ_KAMAITACHI"],
		[416,10,"コインの枚数"],
		[417,1,"フライング","GS_FLING"],
		[418,1,"トリプルアクション","GS_TRIPLEACTION"],
		[419,1,"ブルズアイ","GS_BULLSEYE"],
		[420,1,"マッドネスキャンセラー","GS_MADNESSCANCEL"],
		[421,1,"アジャストメント","GS_ADJUSTMENT"],
		[422,1,"インクリージングアキュラシー","GS_INCREASING"],
		[423,1,"マジカルバレット","GS_MAGICALBULLET"],
		[424,1,"クラッカー","GS_CRACKER"],
		[425,10,"シングルアクション","GS_SINGLEACTION"],
		[426,10,"スネークアイ","GS_SNAKEEYE"],
		[427,10,"チェーンアクション","GS_CHAINACTION"],
		[428,10,"ラピッドシャワー","GS_RAPIDSHOWER"],
		[429,10,"デスペラード","GS_DESPERADO"],
		[430,10,"トラッキング","GS_TRACKING"],
		[431,5,"ディスアーム","GS_DISARM"],
		[432,5,"ピアーシングショット","GS_PIERCINGSHOT"],
		[433,10,"ガトリングフィーバー","GS_GATLINGFEVER"],
		[434,10,"ダスト"],
		[435,10,"フルバスター","GS_FULLBUSTER"],
		[436,10,"スプレッドアタック","GS_SPREADATTACK"],
		[437,10,"グラウンドドリフト","GS_GROUNDDRIFT"],
		[438,10,"一閃(MaxHP固定)"],
		[439,10,"エンチャントブレイド","RK_ENCHANTBLADE"],
		[440,10,"ソニックウェーブ","RK_SONICWAVE"],
		[441,10,"(△)デスバウンド","RK_DEATHBOUND"],
		[442,10,"ハンドレッドスピア","RK_HUNDREDSPEAR"],
		[443,5,"ウィンドカッター","RK_WINDCUTTER"],
		[444,5,"ファントムスラスト","RK_PHANTOMTHRUST"],
		[445,5,"(仮)イグニッションブレイク","RK_IGNITIONBREAK"],
		[446,5,"ドラゴントレーニング","RK_DRAGONTRAINING"],
		[447,10,"(△)ファイアードラゴンブレス","RK_DRAGONBREATH"],
		[448,5,"ドラゴンハウリング","RK_DRAGONHOWLING"],
		[449,10,"ルーンマスタリー","RK_RUNEMASTERY"],
		[450,1,"ジャイアントグロース"],
		[451,1,"バイタリティアクティベーション"],
		[452,1,"ストームブラスト"],
		[453,1,"ストーンハードスキン"],
		[454,1,"ファイティングスピリット"],
		[455,1,"アバンダンス"],
		[456,1,"クラッシュストライク"],
		[457,1,"リフレッシュ"],
		[458,1,"ミレニアムシールド"],
		[459,5,"ベナムインプレス","GC_VENOMIMPRESS"],
		[460,5,"クロスインパクト","GC_CROSSIMPACT"],
		[461,5,"ダークイリュージョン","GC_DARKILLUSION"],
		[462,10,"新毒研究","GC_RESEARCHNEWPOISON"],
		[463,1,"新毒製造","GC_CREATENEWPOISON"],
		[464,1,"アンチドート","GC_ANTIDOTE"],
		[465,5,"ポイズニングウェポン","GC_POISONINGWEAPON"],
		[466,5,"ベナムプレッシャー","GC_VENOMPRESSURE"],
		[467,5,"ポイズンスモーク","GC_POISONSMOKE"],
		[468,5,"(×)ウェポンブロッキング","GC_WEAPONBLOCKING"],
		[469,10,"(△)カウンタースラッシュ","GC_COUNTERSLASH"],
		[470,5,"ウェポンクラッシュ","GC_WEAPONCRUSH"],
		[471,5,"クローキングエクシード","GC_CLOAKINGEXCEED"],
		[472,1,"ファントムメナス","GC_PHANTOMMENACE"],
		[473,5,"ハルシネーションウォーク","GC_HALLUCINATIONWALK"],
		[474,5,"ローリングカッター","GC_ROLLINGCUTTER"],
		[475,5,"クロスリッパースラッシャー","GC_CROSSRIPPERSLASHER"],
		[476,10,"(△)ジュデックス","AB_JUDEX"],
		[477,1,"アンシラ","AB_ANCILLA"],
		[478,10,"アドラムス","AB_ADORAMUS"],
		[479,3,"クレメンティア","AB_CLEMENTIA"],
		[480,3,"カントキャンディダス","AB_CANTO"],
		[481,3,"コルセオヒール","AB_CHEAL"],
		[482,5,"エピクレシス","AB_EPICLESIS"],
		[483,10,"プラエファティオ","AB_PRAEFATIO"],
		[484,10,"オラティオ","AB_ORATIO"],
		[485,4,"ラウダアグヌス","AB_LAUDAAGNUS"],
		[486,4,"ラウダラムス","AB_LAUDARAMUS"],
		[487,10,"(廃止)エウカリスティカ","AB_EUCHARISTICA"],
		[488,4,"レノヴァティオ","AB_RENOVATIO"],
		[489,5,"ハイネスヒール","AB_HIGHNESSHEAL"],
		[490,5,"クリアランス","AB_CLEARANCE"],
		[491,5,"エクスピアティオ","AB_EXPIATIO"],
		[492,10,"デュプレライト","AB_DUPLELIGHT"],
		[493,5,"シレンティウム","AB_SILENTIUM"],
		[494,5,"サクラメント","AB_SECRAMENT"],
		[495,10,"レンジャーメイン","RA_RANGERMAIN"],
		[496,5,"カモフラージュ","RA_CAMOUFLAGE"],
		[497,10,"エイムドボルト","RA_AIMEDBOLT"],
		[498,10,"アローストーム","RA_ARROWSTORM"],
		[499,5,"フィアーブリーズ","RA_FEARBREEZE"],
		[500,10,"トラップ研究","RA_RESEARCHTRAP"],
		[501,1,"マゼンタトラップ","RA_MAGENTATRAP"],
		[502,1,"コバルトトラップ","RA_COBALTTRAP"],
		[503,1,"ヴェルデュールトラップ","RA_VERDURETRAP"],
		[504,1,"メイズトラップ","RA_MAIZETRAP"],
		[505,5,"クラスターボム","RA_CLUSTERBOMB"],
		[506,1,"デトネイター","RA_DETONATOR"],
		[507,5,"ファイアリングトラップ","RA_FIRINGTRAP"],
		[508,5,"アイスバウンドトラップ","RA_ICEBOUNDTRAP"],
		[509,5,"エレクトリックショッカー","RA_ELECTRICSHOCKER"],
		[510,1,"ウォーグマスタリー","RA_WUGMASTERY"],
		[511,5,"ウォーグバイト","RA_WUGBITE"],
		[512,10,"トゥースオブウォーグ","RA_TOOTHOFWUG"],
		[513,5,"ウォーグストライク","RA_WUGSTRIKE"],
		[514,5,"鋭敏な嗅覚","RA_SENSITIVEKEEN"],
		[515,3,"ウォーグライダー","RA_WUGRIDER"],
		[516,1,"ウォーグダッシュ","RA_WUGDASH"],
		[517,5,"ホワイトインプリズン","WL_WHITEIMPRISON"],
		[518,5,"ソウルエクスパンション","WL_SOULEXPANSION"],
		[519,5,"フロストミスティ","WL_FROSTMISTY"],
		[520,5,"ジャックフロスト","WL_JACKFROST"],
		[521,5,"マーシュオブアビス","WL_MARSHOFABYSS"],
		[522,5,"リコグナイズドスペル","WL_RECOGNIZEDSPELL"],
		[523,5,"シエナエクセクレイト","WL_SIENNAEXECRATE"],
		[524,3,"ラディウス","WL_RADIUS"],
		[525,5,"ステイシス","WL_STASIS"],
		[526,5,"ドレインライフ","WL_DRAINLIFE"],
		[527,5,"クリムゾンロック","WL_CRIMSONROCK"],
		[528,5,"ヘルインフェルノ","WL_HELLINFERNO"],
		[529,5,"コメット","WL_COMET"],
		[530,5,"チェーンライトニング","WL_CHAINLIGHTNING"],
		[531,5,"アースストレイン","WL_EARTHSTRAIN"],
		[532,10,"テトラボルテックス","WL_TETRAVORTEX"],
		[533,5,"サモンファイアーボール","WL_SUMMONFB"],
		[534,5,"サモンウォーターボール","WL_SUMMONWB"],
		[535,5,"サモンボールライトニング","WL_SUMMONBL"],
		[536,5,"サモンストーン","WL_SUMMONSTONE"],
		[537,2,"リリース","WL_RELEASE"],
		[538,1,"リーディングスペルブック","WL_READING_SB"],
		[539,10,"フリージングスペル","WL_FREEZE_SP"],
		[540,10,"斧鍛錬","NC_TRAININGAXE"],
		[541,5,"アックストルネード","NC_AXETORNADO"],
		[542,5,"アックスブーメラン","NC_AXEBOOMERANG"],
		[543,10,"(△)パワースイング","NC_POWERSWING"],
		[544,5,"火と大地の研究","NC_RESEARCHFE"],
		[545,5,"FAW シルバースナイパー","NC_SILVERSNIPER"],
		[546,5,"FAW マジックデコイ","NC_MAGICDECOY"],
		[547,1,"FAW 解体","NC_DISJOINT"],
		[548,5,"魔導ギアライセンス","NC_MADOLICENCE"],
		[549,5,"ブーストナックル","NC_BOOSTKNUCKLE"],
		[550,3,"パイルバンカー","NC_PILEBUNKER"],
		[551,3,"バルカンアーム","NC_VULCANARM"],
		[552,3,"フレイムスローワー","NC_FLAMELAUNCHER"],
		[553,3,"コールドスローワー","NC_COLDSLOWER"],
		[554,5,"(△)アームズキャノン","NC_ARMSCANNON"],
		[555,3,"アクセラレーション","NC_ACCELERATION"],
		[556,1,"ホバーリング","NC_HOVERING"],
		[557,1,"フロントサイドスライド","NC_F_SIDESLIDE"],
		[558,1,"リアサイドスライド","NC_B_SIDESLIDE"],
		[559,4,"メインフレーム改造","NC_MAINFRAME"],
		[560,4,"シェイプシフト","NC_SHAPESHIFT"],
		[561,1,"インフラレッドスキャン","NC_INFRAREDSCAN"],
		[562,3,"アナライズ","NC_ANALYZE"],
		[563,3,"セルフディストラクション","NC_SELFDESTRUCTION"],
		[564,1,"エマージェンシークール","NC_EMERGENCYCOOL"],
		[565,3,"マグネティックフィールド","NC_MAGNETICFIELD"],
		[566,3,"ニュートラルバリアー","NC_NEUTRALBARRIER"],
		[567,3,"ステルスフィールド","NC_STEALTHFIELD"],
		[568,5,"リペア","NC_REPAIR"],
		[569,5,"キャノンスピア","LG_CANNONSPEAR"],
		[570,10,"バニシングポイント","LG_BANISHINGPOINT"],
		[571,3,"トランプル","LG_TRAMPLE"],
		[572,10,"(△)シールドプレス","LG_SHIELDPRESS"],
		[573,5,"リフレクトダメージ","LG_REFLECTDAMAGE"],
		[574,5,"ピンポイントアタック","LG_PINPOINTATTACK"],
		[575,5,"フォースオブバンガード","LG_FORCEOFVANGUARD"],
		[576,1,"レイジバーストアタック","LG_RAGEBURST"],
		[577,3,"シールドスペル","LG_SHIELDSPELL"],
		[578,5,"イクシードブレイク","LG_EXEEDBREAK"],
		[579,5,"オーバーブランド","LG_OVERBRAND"],
		[580,5,"プレスティージ","LG_PRESTIGE"],
		[581,5,"バンディング","LG_BANDING"],
		[582,5,"ムーンスラッシャー","LG_MOONSLASHER"],
		[583,10,"(△)レイオブジェネシス","LG_RAYOFGENESIS"],
		[584,5,"パイエティ","LG_PIETY"],
		[585,5,"アースドライブ","LG_EARTHDRIVE"],
		[586,5,"(仮)ヘスペルスリット","LG_HESPERUSLIT"],
		[587,5,"インスピレーション","LG_INSPIRATION"],
		[588,5,"ボディペインティング","SC_BODYPAINT"],
		[589,3,"マスカレード-エナベーション","SC_ENERVATION"],
		[590,3,"マスカレード-グルーミー","SC_GROOMY"],
		[591,3,"マスカレード-イグノアランス","SC_IGNORANCE"],
		[592,3,"マスカレード-レイジネス","SC_LAZINESS"],
		[593,3,"マスカレード-ウィークネス","SC_WEAKNESS"],
		[594,3,"マスカレード-アンラッキー","SC_UNLUCKY"],
		[595,10,"リプロデュース","SC_REPRODUCE"],
		[596,10,"(仮)オートシャドウスペル","SC_AUTOSHADOWSPELL"],
		[597,5,"シャドウフォーム","SC_SHADOWFORM"],
		[598,5,"デッドリーインフェクト","SC_DEADLYINFECT"],
		[599,5,"(仮)インビジビリティ","SC_INVISIBILITY"],
		[600,3,"マンホール","SC_MANHOLE"],
		[601,3,"ディメンションドア","SC_DIMENSIONDOOR"],
		[602,3,"ブラッディラスト","SC_BLOODYLUST"],
		[603,10,"(△)フェイントボム","SC_FEINTBOMB"],
		[604,3,"カオスパニック","SC_CHAOSPANIC"],
		[605,3,"メイルストーム","SC_MAELSTROM"],
		[606,10,"フェイタルメナス","SC_FATALMENACE"],
		[607,5,"ストリップアクセサリー","SC_STRIPACCESSARY"],
		[608,10,"トライアングルショット","SC_TRIANGLESHOT"],
		[609,10,"双龍脚","SR_DRAGONCOMBO"],
		[610,5,"天羅地網","SR_SKYNETBLOW"],
		[611,5,"地雷震","SR_EARTHSHAKER"],
		[612,5,"爆気散弾","SR_RAMPAGEBLASTER"],
		[613,10,"(△)修羅身弾","SR_KNUCKLEARROW"],
		[614,10,"(△)大纏崩捶","SR_FALLENEMPIRE"],
		[615,10,"號砲","SR_TIGERCANNON"],
		[616,10,"羅刹破凰撃(HPSP固定)"],
		[617,10,"羅刹破凰撃(HPSP変動可)","SR_GATEOFHELL"],
		[618,1,"旋風腿","SR_WINDMILL"],
		[619,5,"呪縛陣","SR_CURSEDCIRCLE"],
		[620,5,"閃電歩","SR_LIGHTNINGWALK"],
		[621,10,"潜龍昇天","SR_RAISINGDRAGON"],
		[622,5,"獅子吼","SR_HOWLINGOFLION"],
		[623,5,"雷光弾","SR_RIDEINLIGHTNING"],
		[624,5,"点穴 -黙-","SR_GENTLETOUCH_QUIET"],
		[625,5,"点穴 -快-","SR_GENTLETOUCH_CURE"],
		[626,5,"点穴 -球-","SR_GENTLETOUCH_ENERGYGAIN"],
		[627,5,"点穴 -反-","SR_GENTLETOUCH_CHANGE"],
		[628,5,"点穴 -活-","SR_GENTLETOUCH_REVITALIZE"],
		[629,1,"吸気功","SR_ASSIMILATEPOWER"],
		[630,5,"破碎柱","SR_CRESCENTELBOW"],
		[631,10,"レッスン","WM_LESSON"],
		[632,5,"安らぎの子守唄","WM_LULLABY_DEEPSLEEP"],
		[633,5,"地獄の歌","WM_POEMOFNETHERWORLD"],
		[634,5,"不確定要素の言語","WM_RANDOMIZESPELL"],
		[635,5,"メランコリー","WM_GLOOMYDAY"],
		[636,5,"セイレーンの声","WM_VOICEOFSIREN"],
		[637,5,"循環する自然の音","WM_SIRCLEOFNATURE"],
		[638,5,"生死の境で","WM_DEADHILLHERE"],
		[639,5,"振動残響","WM_REVERBERATION"],
		[640,1,"ドミニオンインパルス","WM_DOMINION_IMPULSE"],
		[641,10,"(△)メタリックサウンド","WM_METALICSOUND"],
		[642,5,"シビアレインストーム","WM_SEVERE_RAINSTORM"],
		[643,5,"風車に向かって突撃","MI_RUSH_WINDMILL"],
		[644,5,"エコーの歌","MI_ECHOSONG"],
		[645,5,"ハーモナイズ","MI_HARMONIZE"],
		[646,5,"月明かりのセレナーデ","WA_MOONLIT_SERENADE"],
		[647,5,"恋人たちの為のシンフォニー","WA_SYMPHONY_OF_LOVER"],
		[648,5,"スイングダンス","WA_SWING_DANCE"],
		[649,5,"レーラズの霧","WM_LERADS_DEW"],
		[650,5,"ビヨンドオブウォークライ","WM_BEYOND_OF_WARCRY"],
		[651,5,"マナの歌","WM_SONG_OF_MANA"],
		[652,5,"メロディーオブシンク","WM_MELODYOFSINK"],
		[653,5,"ダンスウィズウォーグ","WM_DANCE_WITH_WUG"],
		[654,5,"フライデーナイトフィーバー","WM_SATURDAY_NIGHT_FEVER"],
		[655,5,"サウンドオブディストラクション","WM_SOUND_OF_DESTRUCTION"],
		[656,5,"エンドレスハミングボイス","WM_UNLIMITED_HUMMING_VOICE"],
		[657,5,"(×)グレートエコー(未検証)","WM_GREAT_ECHO"],
		[658,5,"ファイアーウォーク","SO_FIREWALK"],
		[659,5,"エレクトリックウォーク","SO_ELECTRICWALK"],
		[660,10,"(△)スペルフィスト","SO_SPELLFIST"],
		[661,5,"バキュームエクストリーム","SO_VACUUM_EXTREME"],
		[662,5,"サイキックウェーブ","SO_PSYCHIC_WAVE"],
		[663,5,"クラウドキル","SO_CLOUD_KILL"],
		[664,5,"ポイズンバスター","SO_POISON_BUSTER"],
		[665,5,"ストライキング","SO_STRIKING"],
		[666,5,"アースグレイヴ","SO_EARTHGRAVE"],
		[667,5,"ダイヤモンドダスト","SO_DIAMONDDUST"],
		[668,5,"ウォーマー","SO_WARMER"],
		[669,10,"(△)ヴェラチュールスピアー","SO_VARETYR_SPEAR"],
		[670,5,"アルージョ","SO_ARRULLO"],
		[671,3,"サモンアグニ","SO_SUMMON_AGNI"],
		[672,3,"サモンアクア","SO_SUMMON_AQUA"],
		[673,3,"サモンベントス","SO_SUMMON_VENTUS"],
		[674,3,"サモンテラ","SO_SUMMON_TERA"],
		[675,4,"エレメンタルコントロール","SO_EL_CONTROL"],
		[676,1,"エレメンタルアクション","SO_EL_ACTION"],
		[677,2,"エレメンタルアナライシス","SO_EL_ANALYSIS"],
		[678,5,"エレメンタルシンパシー","SO_EL_SYMPATHY"],
		[679,1,"エレメンタルキュアー","SO_EL_CURE"],
		[680,3,"ファイアーインシグニア","SO_FIRE_INSIGNIA"],
		[681,3,"ウォーターインシグニア","SO_WATER_INSIGNIA"],
		[682,3,"ウィンドインシグニア","SO_WIND_INSIGNIA"],
		[683,3,"アースインシグニア","SO_EARTH_INSIGNIA"],
		[684,1,"パイロテクニック"],
		[685,1,"サークルオブファイアー"],
		[686,1,"ファイアーアロー"],
		[687,1,"ヒーター"],
		[688,1,"ファイアークローク"],
		[689,1,"ファイアーボム"],
		[690,1,"トロピック"],
		[691,1,"ファイアーマントル"],
		[692,1,"ファイアーウェーブ"],
		[693,1,"アクアプレイ"],
		[694,1,"ウォータースクリーン"],
		[695,1,"アイスニードル"],
		[696,1,"クーラー"],
		[697,1,"ウォータードロップ"],
		[698,1,"ウォータースクリュー"],
		[699,1,"チリエア"],
		[700,1,"ウォーターバリア"],
		[701,1,"タイダルウェポン"],
		[702,1,"ガスト"],
		[703,1,"ウィンドステップ"],
		[704,1,"ウィンドスラッシュ"],
		[705,1,"ブラスト"],
		[706,1,"ウィンドカーテン"],
		[707,1,"ハリケーンレイジ"],
		[708,1,"ワイルドストーム"],
		[709,1,"ゼファー"],
		[710,1,"タイフーンミサイル"],
		[711,1,"ペトロロジー"],
		[712,1,"ソリッドスキン"],
		[713,1,"ストーンハンマー"],
		[714,1,"カーズドソイル"],
		[715,1,"ストーンシールド"],
		[716,1,"ロッククラッシャー"],
		[717,1,"アップヒーバル"],
		[718,1,"パワーオブガイア"],
		[719,1,"ストーンレイン"],
		[720,5,"剣鍛錬","GN_TRAINING_SWORD"],
		[721,5,"カート改造","GN_REMODELING_CART"],
		[722,10,"(△)カートトルネード","GN_CART_TORNADO"],
		[723,5,"カートキャノン","GN_CARTCANNON"],
		[724,5,"改造カートブースト","GN_CARTBOOST"],
		[725,1,"チェンジマテリアル","GN_CHANGEMATERIAL"],
		[726,1,"スリングアイテム","GN_SLINGITEM"],
		[727,10,"スペシャルファーマシー","GN_S_PHARMACY"],
		[728,2,"ミックスクッキング","GN_MIX_COOKING"],
		[729,2,"爆弾製造","GN_MAKEBOMB"],
		[730,5,"ソーントラップ","GN_THORNS_TRAP"],
		[731,5,"ソーンウォール","GN_WALLOFTHORN"],
		[732,10,"クレイジーウィード","GN_CRAZYWEED"],
		[733,5,"ブラッドサッカー","GN_BLOOD_SUCKER"],
		[734,5,"ヘルズプラント","GN_HELLS_PLANT"],
		[735,5,"ハウリングオブマンドラゴラ","GN_MANDRAGORA"],
		[736,10,"スポアエクスプロージョン","GN_SPORE_EXPLOSION"],
		[737,5,"デモニックファイアー","GN_DEMONIC_FIRE"],
		[738,5,"(×)ファイアーエクスパンション(Lv5)","GN_FIRE_EXPANSION"],
		[739,1,"魔導ギア"],
		[740,3,"セルフディストラクション(HPSP固定)"],
		[741,10,"グレイアムライト"],
		[742,10,"ミリアムライト"],
		[743,3,"AS用設定魔法"],
		[744,1,"シールドスペル(ATK+)"],
		[745,1,"シールドスペル(DEF+)"],
		[746,5,"ディフェンダーの習得Lv(プレスティージ用)"],
		[747,1,"ASS用設定魔法"],
		[748,1,"全気注入","SR_POWERVELOCITY"],
		[749,10,"セージの魂(魔法の習得Lv)"],
		[750,1,"ハルシネーション効果後のASPD減"],
		[751,5,"自動狼"],
		[752,1,"(特殊)EDP毒部分を消す[通常はoff]"],
		[753,5,"シビアレインストーム(特殊)"],
		[754,5,"ロイヤルガードの人数(バンディング用)"],
		[755,5,"ダブルキャスティング","PF_DOUBLECASTING"],
		[756,1,"シールドスペル(反射)"],
		[757,5,"マナリチャージ","HP_MANARECHARGE"],
		[758,1,"シールドスペルLv1(物理)"],
		[759,1,"シールドスペルLv2(魔法)"],
		[760,5,"致命的な傷"],
		[761,5,"アイアンネイル用ATK+"],
		[762,10,"ヘルジャッジメント"],
		[763,1,"闇雲","KO_YAMIKUMO"],
		[764,5,"右手鍛錬","KO_RIGHT"],
		[765,5,"左手鍛錬","KO_LEFT"],
		[766,10,"(△)十文字斬り","KO_JYUMONJIKIRI"],
		[767,5,"黄泉返し","KO_SETSUDAN"],
		[768,5,"(△)爆裂苦無","KO_BAKURETSU"],
		[769,5,"八方苦無","KO_HAPPOKUNAI"],
		[770,10,"風魔手裏剣 -乱華-","KO_HUUMARANKA"],
		[771,5,"撒菱","KO_MAKIBISHI"],
		[772,10,"(仮)無茶投げ","KO_MUCHANAGE"],
		[773,5,"明鏡止水","KO_MEIKYOUSISUI"],
		[774,5,"幻術-影武者-","KO_ZANZOU"],
		[775,5,"幻術-驚愕-","KO_KYOUGAKU"],
		[776,5,"幻術-呪殺-","KO_JYUSATSU"],
		[777,5,"幻術-幻惑-","KO_GENWAKU"],
		[778,5,"十六夜","KO_IZAYOI"],
		[779,10,"(×)火符：炎天","KO_KAHU_ENTEN"],
		[780,10,"(×)氷符：吹雪","KO_HYOUHU_HUBUKI"],
		[781,10,"(×)風符：青嵐","KO_KAZEHU_SEIRAN"],
		[782,10,"(×)土符：剛塊","KO_DOHU_KOUKAI"],
		[783,1,"術式-解放-","KO_KAIHOU"],
		[784,1,"術式-展開-","KO_ZENKAI"],
		[785,5,"幻術-影踏み-","KG_KAGEHUMI"],
		[786,5,"幻術-虚無の影-","KG_KYOMU"],
		[787,5,"(×)幻術-分身-","KG_KAGEMUSYA"],
		[788,5,"幻術-残月-","OB_ZANGETSU"],
		[789,5,"幻術-紅月-","OB_AKAITSUKI"],
		[790,5,"幻術-朧幻想-","OB_OBOROGENSOU"],
		[791,4,"符の属性"],
		[792,10,"符の数"],
		[793,1,"残月用HpSp設定(前Hp後Sp 偶=偶数 奇=奇数)"],
		[794,10,"(△)ウォータードラゴンブレス","RK_DRAGONBREATH_WATER"],
		[795,5,"アンリミット","RA_UNLIMIT"],
		[796,5,"オフェルトリウム","AB_OFFERTORIUM"],
		[797,5,"ダーククロー","GC_DARKCROW"],
		[798,5,"テレキネシスインテンス","WL_TELEKINESIS_INTENSE"],
		[799,5,"閃光連撃","SR_FLASHCOMBO"],
		[800,1,"(仮)コンボ計算(三段～)"],
		[801,1,"(仮)コンボ計算(三段～)"],
		[802,1,"(仮)コンボ計算(双龍～)"],
		[803,1,"(仮)コンボ計算(～)"],
		[804,1,"(仮)コンボ計算(～)"],
		[805,1,"(仮)コンボ計算(～)"],
		[806,1,"(仮)コンボ計算(～)"],
		[807,1,"(仮)コンボ計算(～)"],
		[808,1,"(仮)コンボ計算(～)"],
		[809,1,"(仮)コンボ計算(～)"],
		[810,10,"(仮)アースクエイク"],
		[811,5,"マグマイラプション","NC_MAGMA_ERUPTION"],
		[812,1,"(仮)精霊"],
		[813,1,"精霊(モード)"],
		[814,1,"(仮)精霊(補助スキル)"],
		[815,1,"SホムのLv(パイロ用)"],
		[816,5,"パイロクラスティック(Sホム)"],
		[817,5,"オーバードブースト(Sホム)"],
		[818,5,"(仮)グラニティックアーマー(Sホム)"],
		[819,5,"(仮)ペインキラー(Sホム)"],
		[820,5,"ディフェンス(ホム)"],
		[821,5,"閃光連撃終了直後状態(ATK+状態)"],
		[822,1,"リッチズコイン","RL_RICHS_COIN"],
		[823,1,"フォーリンエンジェル","RL_FALLEN_ANGEL"],
		[824,5,"シャッターストーム","RL_S_STORM"],
		[825,5,"マススパイラル","RL_MASS_SPIRAL"],
		[826,10,"エターナルチェーン","RL_E_CHAIN"],
		[827,5,"ハウリングマイン","RL_H_MINE"],
		[828,5,"ファイアーレイン","RL_FIRE_RAIN"],
		[829,1,"フリッカー","RL_FLICKER"],
		[830,10,"ファイアーダンス","RL_FIREDANCE"],
		[831,5,"バニシングバスター","RL_BANISHING_BUSTER"],
		[832,5,"アンチマテリアルブラスト","RL_AM_BLAST"],
		[833,1,"クイックドローショット","RL_QD_SHOT"],
		[834,10,"ドラゴンテイル","RL_D_TAIL"],
		[835,10,"ラウンドトリップ","RL_R_TRIP"],
		[836,5,"ヒートバレル","RL_HEAT_BARREL"],
		[837,10,"ヒートバレルのコイン枚数"],
		[838,5,"スラッグショット","RL_SLUGSHOT"],
		[839,10,"ハンマーオブゴッド","RL_HAMMER_OF_GOD"],
		[840,1,"クリムゾンマーカー","RL_C_MARKER"],
		[841,5,"プラチナムアルター","RL_P_ALTER"],
		[842,10,"プラチナムのコイン枚数"],
		[843,5,"バインドトラップ","RL_B_TRAP"],
		[844,5,"ハウリングマイン追撃"],
		[845,1,"クイックドローショットの全追撃"],
		[846,9,"基本スキル"],
		[847,1,"レディムプティオ","PR_REDEMPTIO"],
		[848,1,"サイトブラスター","WZ_SIGHTBLASTER"],
		[849,1,"グリード","BS_GREED"],
		[850,1,"フェイクゼニー","BS_UNFAIRLYTRICK"],
		[851,10,"オートガード"],
		[852,1,"シュリンク"],
		[853,1,"気功転移","MO_KITRANSLATION"],
		[854,1,"クリエイトコンバータ","SA_CREATECON"],
		[855,1,"ファイアーエレメンタルチェンジ","SA_ELEMENTFIRE"],
		[856,1,"ウォーターエレメンタルチェンジ","SA_ELEMENTWATER"],
		[857,1,"ウィンドエレメンタルチェンジ","SA_ELEMENTWIND"],
		[858,1,"アースエレメンタルチェンジ","SA_ELEMENTGROUND"],
		[859,1,"生命倫理","AM_BIOETHICS"],
		[860,1,"安息","AM_REST"],
		[861,1,"コールホムンクルス","AM_CALLHOMUN"],
		[862,5,"リザレクションホムンクルス","AM_RESURRECTHOMUN"],
		[863,1,"ガンバンテイン","HW_GANBANTEIN"],
		[864,10,"武器精錬","WS_WEAPONREFINE"],
		[865,5,"プレッシャー（重複）"],
		[866,5,"フルストリップ","ST_FULLSTRIP"],
		[867,1,"プリザーブ","ST_PRESERVE"],
		[868,1,"音楽専門家の熟練","CG_SPECIALSINGER"],
		[869,5,"ヘルモードの杖","CG_HERMODE"],
		[870,5,"月明かりの下で","CG_MOONLIT"],
		[871,5,"生命力変換","PF_HPCONVERSION"],
		[872,1,"スパイダーウェブ","PF_SPIDERWEB"],
		[873,1,"ウォールオブフォグ","PF_FOGWALL"],
		[874,10,"スリムポーションピッチャー","CR_SLIMPITCHER"],
		[875,5,"フルケミカルチャージ","CR_FULLPROTECTION"],
		[876,2,"植物栽培","CR_CULTIVATION"],
		[877,5,"ナイトの魂","SL_KNIGHT"],
		[878,5,"アサシンの魂","SL_ASSASIN"],
		[879,5,"プリーストの魂","SL_PRIEST"],
		[880,5,"ハンターの魂","SL_HUNTER"],
		[881,5,"ウィザードの魂","SL_WIZARD"],
		[882,5,"ブラックスミスの魂","SL_BLACKSMITH"],
		[883,5,"クルセイダーの魂","SL_CRUSADER"],
		[884,5,"ローグの魂","SL_ROGUE"],
		[885,5,"モンクの魂","SL_MONK"],
		[886,5,"バードとダンサーの魂","SL_BARDDANCER"],
		[887,5,"セージの魂","SL_SAGE"],
		[888,5,"アルケミストの魂","SL_ALCHEMIST"],
		[889,5,"拳聖の魂","SL_STAR"],
		[890,5,"ソウルリンカーの魂","SL_SOULLINKER"],
		[891,5,"フリップザコイン","GS_GLITTERING"],
		[892,5,"キングスグレイス","LG_KINGS_GRACE"],
		[893,5,"エスケープ","SC_ESCAPE"],
		[894,5,"フリッグの歌","WM_FRIGG_SONG"],
		[895,5,"エレメンタルシールド","SO_ELEMENTAL_SHIELD"],
		[896,5,"イリュージョンドーピング","GN_ILLUSIONDOPING"],
		[897,10,"ダーククロス"],
		[898,10,"インティミデイト(盗作用Ex)"],
		[899,1,"(仮)コンボ計算(ｼﾞｮｲﾝﾄ→SpP→ｿﾆｯｸ)"],
		[900,1,"(仮)コンボ計算(SpP→ｿﾆｯｸ)"],
		[901,1,"フルスロットル","EVT_FULL_THROTTLE"],
		[902,1,"ドラム基本スキル","SU_BASIC_SKILL"],
		[903,1,"かみつく","SU_BITE"],
		[904,1,"かくれる","SU_HIDE"],
		[905,3,"ひっかく","SU_SCRATCH"],
		[906,1,"うずくまる","SU_STOOP"],
		[907,3,"ニャンジャンプ","SU_LOPE"],
		[908,1,"にゃん魂","SU_SPRITEMABLE"],
		[909,1,"ソウルアタック","SU_SOULATTACK"],
		[910,5,"新鮮なエビ","SU_FRESHSHRIMP"],
		[911,5,"エビ三昧","SU_BUNCHOFSHRIMP"],
		[912,5,"大トロ","SU_TUNABELLY"],
		[913,5,"マグロシールド","SU_TUNAPARTY"],
		[914,1,"海の力","SU_POWEROFSEA"],
		[915,50,"シーフード系習得レベル合計"],
		[916,5,"グルーミング","SU_GROOMING"],
		[917,5,"のどを鳴らす","SU_PURRING"],
		[918,5,"エビパーティー","SU_SHRIMPARTY"],
		[919,1,"海の魂","SU_SPIRITOFSEA"],
		[920,5,"マタタビランス","SU_SV_STEMSPEAR"],
		[921,5,"マタタビの根っこ","SU_SV_ROOTTWIST"],
		[922,5,"イヌハッカメテオ","SU_CN_METEOR"],
		[923,5,"(×)イヌハッカシャワー","SU_CN_POWDERING"],
		[924,1,"大地の力","SU_POWEROFLAND"],
		[925,50,"プラント系習得レベル合計"],
		[926,5,"チャタリング","SU_CHATTERING"],
		[927,5,"ミャウミャウ","SU_MEOWMEOW"],
		[928,5,"ニャングラス","SU_NYANGGRASS"],
		[929,1,"大地の魂","SU_SPIRITOFLAND"],
		[930,5,"ピッキ突き","SU_PICKYPECK"],
		[931,5,"アクラウスダッシュ","SU_ARCLOUSEDASH"],
		[932,5,"タロウの傷","SU_SCAROFTAROU"],
		[933,5,"キャロットビート","SU_LUNATICCARROTBEAT"],
		[934,1,"生命の力","SU_POWEROFLIFE"],
		[935,50,"アニマル系習得レベル合計"],
		[936,5,"警戒","SU_HISS"],
		[937,5,"群れの力","SU_POWEROFFLOCK"],
		[938,5,"サベージの魂","SU_SVG_SPIRIT"],
		[939,1,"生命の魂","SU_SPIRITOFLIFE"],
		[940,1,"大地の魂効果(ﾏﾀﾀﾋﾞの根っこ使用後のMATK＋)"],
		[941,1,"大地の魂効果(ｲﾇﾊｯｶｼｬﾜｰ使用後の完全回避＋)"],
		[942,1,"大地の魂効果(ニャングラス使用後のMATK＋)"],
		[943,1,"生命の魂効果(残りHP)"],
		[944,5,"ブレイクスルー","NV_BREAKTHROUGH"],
		[945,5,"トランセンデンス","NV_TRANSCENDENCE"],
		[946,1,"天使さま助けて","NV_HELPANGEL"],
		[947,3,"太陽と月と星の記録","SJ_DOCUMENT"],
		[948,1,"太陽と月と星の浄化","SJ_PURIFY"],
		[949,3,"太陽の構え","SJ_SUNSTANCE"],
		[950,7,"紅焔脚","SJ_PROMINENCEKICK"],
		[951,10,"太陽爆発","SJ_SOLARBURST"],
		[952,5,"太陽の光","SJ_LIGHTOFSUN"],
		[953,3,"月の構え","SJ_LUNARSTANCE"],
		[954,7,"朔月脚","SJ_NEWMOONKICK"],
		[955,10,"満月脚","SJ_FULLMOONKICK"],
		[956,5,"月の光","SJ_LIGHTOFMOON"],
		[957,3,"星の構え","SJ_STARSTANCE"],
		[958,7,"閃光脚","SJ_FLASHKICK"],
		[959,10,"流星落下","SJ_FALLINGSTAR"],
		[960,5,"星の光","SJ_LIGHTOFSTAR"],
		[961,3,"宇宙の構え","SJ_UNIVERSESTANCE"],
		[962,1,"重力調節","SJ_GRAVITYCONTROL"],
		[963,5,"新星爆発","SJ_NOVAEXPLOSING"],
		[964,5,"星帝降臨","SJ_STAREMPEROR"],
		[965,5,"創星の書","SJ_BOOKOFCREATINGSTAR"],
		[966,5,"次元の書","SJ_BOOKOFDIMENSION"],
		[967,5,"エスハ","SP_SHA"],
		[968,10,"エスパ","SP_SPA"],
		[969,10,"エスフ","SP_SWHOO"],
		[970,5,"カウト","SP_KAUTE"],
		[971,5,"魂の蓄積","SP_SOULCOLLECT"],
		[972,5,"魂の収穫","SP_SOULREAPER"],
		[973,3,"魂の循環","SP_SOULREVOLVE"],
		[974,7,"魂の連結","SP_SOULUNITY"],
		[975,5,"ソウルエナジー研究","SP_SOULENERGY"],
		[976,5,"死霊憑依","SP_SOULCURSE"],
		[977,10,"死霊爆発","SP_CURSEEXPLOSION"],
		[978,5,"(×)魂の分裂","SP_SOULDIVISION"],
		[979,5,"鷹の魂","SP_SOULFALCON"],
		[980,5,"妖精の魂","SP_SOULFAIRY"],
		[981,5,"影の魂","SP_SOULSHADOW"],
		[982,5,"ゴーレムの魂","SP_SOULGOLEM"],
		[983,5,"(×)魂の崩壊","SP_SOULEXPLOSION"],
		[984,20,"ソウルエナジーの個数"],
		[985,2,"流星落下の計算方法"],
		[986,1,"ピオニーマミー"],
		[987,1,"ぴしゃりハーブ"],
		[988,1,"世界樹のほこり"],
		[989,1,"スノーフリップ"],
		[990,10,"所持限界量増加Ｒ"],
		[991,10,"(△)ヴァンパイアギフト"],
		[992,1,"魅惑のウィンク","DC_WINKCHARM"],
		[993,1,"シーフード系スキル"],
		[994,1,"プラント系スキル"],
		[995,1,"アニマル系スキル"],
		[996,10,"ストーンスキン"],
		[997,5,"クリティカルウーンズ"],
		[998,2,"オーディンの力"],
		[999,1,"通常攻撃"],
		[1000,1,"通常攻撃"],
		[1001,1,"x"],
		[1002,5,"サーヴァントウェポン","DK_SERVANTWEAPON"],	// 公式サイトでは「武器体攻撃」と表記
		[1003,5,"サーヴァントウェポン：サイン","DK_SERVANT_W_SIGN"],
		[1004,5,"サーヴァントウェポン：ファントム","DK_SERVANT_W_PHANTOM"],
		[1005,5,"サーヴァントウェポン：デモリッション","DK_SERVANT_W_DEMOL"],
		[1006,10,"チャージングピアース","DK_CHARGINGPIERCE"],
		[1007,10,"ツーハンドディフェンディング","DK_TWOHANDDEF"],
		[1008,10,"ハックアンドスラッシャー","DK_HACKANDSLASHER"],
		[1009,10,"ドラゴニックオーラ","DK_DRAGONIC_AURA"],
		[1010,5,"マッドネスクラッシャー","DK_MADNESS_CRUSHER"],
		[1011,10,"ヴィゴール","DK_VIGOR"],
		[1012,5,"ストームスラッシュ","DK_STORMSLASH"],
		[1013,5,"ダンシングナイフ","SHC_DANCING_KNIFE"],
		[1014,10,"サベージインパクト","SHC_SAVAGE_IMPACT"],
		[1015,10,"シャドウセンス","SHC_SHADOW_SENSE"],
		[1016,5,"エターナルスラッシュ","SHC_ETERNAL_SLASH"],
		[1017,5,"エンチャンティングシャドウ","SHC_ENCHANTING_SHADOW"],
		[1018,10,"ポテントベナム","SHC_POTENT_VENOM"],
		[1019,10,"シャドウエクシード","SHC_SHADOW_EXCEED"],
		[1020,10,"フェイタルシャドウクロー","SHC_FATAL_SHADOW_CROW"],
		[1021,5,"シャドウスタブ","SHC_SHADOW_STAB"],
		[1022,5,"インパクトクレーター","SHC_IMPACT_CRATER"],
		[1023,5,"レパラティオ","CD_REPARATIO"],
		[1024,5,"メディアリボトゥム","CD_MEDIALE_VOTUM"],
		[1025,10,"鈍器＆本修練","CD_MACE_BOOK_M"],
		[1026,5,"アルグトゥスヴィタ","CD_ARGUTUS_VITA"],
		[1027,5,"アルグトゥステルム","CD_ARGUTUS_TELUM"],
		[1028,10,"アルビトリウム","CD_ARBITRIUM"],
		[1029,5,"プレセンスアキエース","CD_PRESENS_ACIES"],
		[1030,10,"フィドスアニムス","CD_FIDUS_ANIMUS"],
		[1031,10,"エフィリゴ","CD_EFFLIGO"],
		[1032,5,"コンペテンティア","CD_COMPETENTIA"],
		[1033,10,"ニューマティックプロセラ","CD_PNEUMATICUS_PROCELLA"],
		[1034,5,"ディレクティオヒール","CD_DILECTIO_HEAL"],
		[1035,5,"(△)レリギオ","CD_RELIGIO"],	// H.Plus +100 ごとの効果量 Up が未実装
		[1036,5,"(△)ベネディクトゥム","CD_BENEDICTUM"],	// H.Plus +100 ごとの効果量 Up が未実装
		[1037,10,"ペティティオ","CD_PETITIO"],
		[1038,10,"ペティティオ習得レベル"],
		[1039,5,"フレーメン","CD_FRAMEN"],
		[1040,5,"アドバンスドトラップ","WH_ADVANCED_TRAP"],
		[1041,5,"(×)ウィンドサイン","WH_WIND_SIGN"],
		[1042,5,"(×)自然親和","WH_NATUREFRIENDLY"],
		[1043,5,"ホークラッシュ","WH_HAWKRUSH"],
		[1044,1,"ホークマスタリー","WH_HAWK_M"],
		[1045,1,"カラミティゲイル","WH_CALAMITYGALE"],
		[1046,5,"ホークブーメラン","WH_HAWKBOOMERANG"],
		[1047,10,"ゲイルストーム","WH_GALESTORM"],
		[1048,5,"ディープブラインドトラップ","WH_DEEPBLINDTRAP"],
		[1049,5,"ソリッドトラップ","WH_SOLIDTRAP"],
		[1050,5,"スイフトトラップ","WH_SWIFTTRAP"],
		[1051,10,"クレッシブボルト","WH_CRESCIVE_BOLT"],
		[1052,5,"フレイムトラップ","WH_FLAMETRAP"],
		[1053,5,"デッドリープロジェクション","AG_DEADLY_PROJECTION"],
		[1054,5,"ディストラクティブハリケーン","AG_DESTRUCTIVE_HURRICANE"],
		[1055,1,"クライマックスハリケーン状態"],
		[1056,5,"レインオブクリスタル","AG_RAIN_OF_CRYSTAL"],
		[1057,5,"ミステリーイリュージョン","AG_MYSTERY_ILLUSION"],
		[1058,5,"バイオレントクエイク","AG_VIOLENT_QUAKE"],
		[1059,5,"ソウルバルカンストライク","AG_SOUL_VC_STRIKE"],
		[1060,5,"ストラタムトレマー","AG_STRANTUM_TREMOR"],
		[1061,5,"オールブルーム","AG_ALL_BLOOM"],
		[1062,5,"クリスタルインパクト","AG_CRYSTAL_IMPACT"],
		[1063,5,"トルネードストーム","AG_TORNADO_STORM"],
		[1064,5,"フローラルフレアロード","AG_FLORAL_FLARE_ROAD"],
		[1065,5,"クライマックス","AG_CLIMAX"],
		[1066,10,"アストラルストライク","AG_ASTRAL_STRIKE"],
		[1067,5,"ロックダウン","AG_ROCK_DOWN"],
		[1068,5,"ストームキャノン","AG_STORM_CANNON"],
		[1069,5,"クリムゾンアロー","AG_CRIMSON_ARROW"],
		[1070,5,"フローズンスラッシュ","AG_FROZEN_SLASH"],
		[1071,10,"両手杖修練","AG_TWOHANDSTAFF"],
		[1072,5,"アックスストンプ","MT_AXE_STOMP"],
		[1073,10,"ラッシュクエイク","MT_RUSH_QUAKE"],
		[1074,10,"ラッシュ状態"],
		[1075,5,"装置製造","MT_M_MACHINE"],
		[1076,5,"攻撃装置有効化","MT_A_MACHINE"],
		[1077,5,"防御装置有効化","MT_D_MACHINE"],
		[1078,10,"ツーアックスディフェンディング","MT_TWOAXEDEF"],
		[1079,10,"(×)ABRマスタリー","MT_ABR_M"],
		[1080,4,"(×)ABR バトルウォリアー","MT_SUMMON_ABR_BATTLE_WARIOR"],
		[1081,4,"ABR デュアルキャノン","MT_SUMMON_ABR_DUAL_CANNON"],
		[1082,4,"(×)ABR マザーネット","MT_SUMMON_ABR_MOTHER_NET"],
		[1083,4,"(×)ABR インフィニティ","MT_SUMMON_ABR_INFINITY"],
		[1084,5,"ガードスタンス","IG_GUARD_STANCE"],
		[1085,5,"ガーディアンシールド","IG_GUARDIAN_SHIELD"],
		[1086,5,"リバウンドシールド","IG_REBOUND_SHIELD"],
		[1087,10,"盾修練","IG_SHIELD_MASTERY"],
		[1088,10,"槍＆片手剣修練","IG_SPEAR_SWORD_M"],
		[1089,5,"アタックスタンス","IG_ATTACK_STANCE"],
		[1090,5,"アルティメットサクリファイス","IG_ULTIMATE_SACRIFICE"],
		[1091,5,"ホーリーシールド","IG_HOLY_SHIELD"],
		[1092,10,"グランドジャッジメント","IG_GRAND_JUDGEMENT"],
		[1093,10,"ジャッジメントクロス","IG_JUDGEMENT_CROSS"],
		[1094,5,"シールドシューティング","IG_SHIELD_SHOOTING"],
		[1095,10,"オーバースラッシュ","IG_OVERSLASH"],
		[1096,10,"クロスレイン","IG_CROSS_RAIN"],
		[1097,10,"短剣＆弓修練","ABC_DAGGER_AND_BOW_M"],
		[1098,10,"魔法剣修練","ABC_MAGIC_SWORD_M"],
		[1099,5,"ストリップシャドウ","ABC_STRIP_SHADOW"],
		[1100,5,"アビスダガー","ABC_ABYSS_DAGGER"],
		[1101,5,"アンラッキーラッシュ","ABC_UNLUCKY_RUSH"],
		[1102,5,"チェーンリアクションショット","ABC_CHAIN_REACTION_SHOT"],
		[1103,5,"フロムジアビス","ABC_FROM_THE_ABYSS"],	// 公式サイトでは「アビス球体攻撃」と表記される
		[1104,10,"アビススレイヤー","ABC_ABYSS_SLAYER"],
		[1105,10,"オメガアビスストライク","ABC_ABYSS_STRIKE"],
		[1106,10,"デフトスタブ","ABC_DEFT_STAB"],
		[1107,5,"アビススクエア","ABC_ABYSS_SQUARE"],
		[1108,5,"アビススクエア習得Lv"],
		[1109,10,"フレンジショット","ABC_FRENZY_SHOT"],
		[1110,5,"強靭な信念","IQ_POWERFUL_FAITH"],
		[1111,5,"堅固な信念","IQ_FIRM_FAITH"],
		[1112,10,"信仰の意志","IQ_WILL_OF_FAITH"],
		[1113,5,"聖油洗礼","IQ_OLEUM_SANCTUM"],
		[1114,5,"忠実な信念","IQ_SINCERE_FAITH"],
		[1115,5,"第一撃：烙印","IQ_FIRST_BRAND"],
		[1116,5,"第一章：信念の力","IQ_FIRST_FAITH_POWER"],
		[1117,5,"第三撃：断罪","IQ_THIRD_PUNISH"],
		[1118,5,"第三撃：滅火撃","IQ_THIRD_FLAME_BOMB"],
		[1119,5,"第三撃：浄化","IQ_THIRD_CONSECRATION"],
		[1120,5,"第二撃：滅魔の火","IQ_SECOND_FLAME"],
		[1121,5,"第二撃：信念","IQ_SECOND_FAITH"],
		[1122,5,"第二撃：審判","IQ_SECOND_JUDGEMENT"],
		[1123,5,"爆火神弾","IQ_EXPOSION_BLASTER"],
		[1124,10,"炎火滅魔神弾","IQ_MASSIVE_F_BLASTER"],
		[1125,5,"第二章：審判者","IQ_JUDGE"],
		[1126,5,"最終章：滅魔の炎","IQ_THIRD_EXOR_FLAME"],
		[1127,5,"ステージマナー","TR_STAGE_MANNER"],
		[1128,1,"回想","TR_RETROSPECTION"],
		[1129,1,"ミスティックシンフォニー","TR_MYSTIC_SYMPHONY"],
		[1130,1,"ソナタオブクヴァシル","TR_KVASIR_SONATA"],
		[1131,5,"ロゼブロッサム","TR_ROSEBLOSSOM"],
		[1132,5,"リズムシューティング","TR_RHYTHMSHOOTING"],
		[1133,5,"メタリックフューリー","TR_METALIC_FURY"],
		[1134,5,"サウンドブレンド","TR_SOUNDBLEND"],
		[1135,5,"(×)ゲフェニアノクターン","TR_GEF_NOCTURN"],
		[1136,5,"(×)ロキの気まぐれ","TR_ROKI_CAPRICCIO"],
		[1137,5,"(×)鉱員のラプソディ","TR_AIN_RHAPSODY"],
		[1138,5,"(△)ミュージカルインタールード","TR_MUSICAL_INTERLUDE"],
		[1139,5,"(△)夕焼けのセレナーデ","TR_JAWAII_SERENADE"],
		[1140,5,"(×)死者たちへのレクイエム","TR_NIPELHEIM_REQUIEM"],
		[1141,5,"(△)プロンテラマーチ","TR_PRON_MARCH"],
		[1142,10,"魔法本修練","EM_MAGIC_BOOK_M"],
		[1143,5,"スペルエンチャンティング","EM_SPELL_ENCHANTING"],
		[1144,5,"アクティビティバーン","EM_ACTIVITY_BURN"],
		[1145,5,"インクリーシングアクティビティ","EM_INCREASING_ACTIVITY"],
		[1146,5,"ダイヤモンドストーム","EM_DIAMOND_STORM"],
		[1147,5,"ライトニングランド","EM_LIGHTNING_LAND"],
		[1148,5,"ベナムスワンプ","EM_VENOM_SWAMP"],
		[1149,5,"コンフラグレーション","EM_CONFLAGRATION"],
		[1150,5,"テラドライブ","EM_TERRA_DRIVE"],
		[1151,10,"エレメンタルスピリットマスタリー","EM_ELEMENTAL_SPIRIT_M"],
		[1152,1,"サモンアルドール","EM_SUMMON_ELEMENTAL_ARDOR"],
		[1153,1,"サモンディルビオ","EM_SUMMON_ELEMENTAL_DILUVIO"],
		[1154,1,"サモンプロセラ","EM_SUMMON_ELEMENTAL_PROCELLA"],
		[1155,1,"サモンテレモトゥス","EM_SUMMON_ELEMENTAL_TERREMOTUS"],
		[1156,1,"サモンサーペンス","EM_SUMMON_ELEMENTAL_SERPENS"],
		[1157,10,"エレメンタルバスター","EM_ELEMENTAL_BUSTER"],
		[1158,5,"エレメンタルヴェール","EM_ELEMENTAL_VEIL"],
		[1159,5,"バイオニックファーマシー","BO_BIONIC_PHARMACY"],
		[1160,10,"バイオニックスマスタリー","BO_BIONICS_M"],
		[1161,5,"ホールフルケミカルチャージ","BO_THE_WHOLE_PROTECTION"],
		[1162,4,"フルシャドウチャージ","BO_ADVANCE_PROTECTION"],
		[1163,5,"アシディファイドゾーン(水)","BO_ACIDIFIED_ZONE_WATER"],
		[1164,5,"アシディファイドゾーン(地)","BO_ACIDIFIED_ZONE_GROUND"],
		[1165,5,"アシディファイドゾーン(火)","BO_ACIDIFIED_ZONE_FIRE"],
		[1166,5,"アシディファイドゾーン(風)","BO_ACIDIFIED_ZONE_WIND"],
		[1167,5,"クリエイトウドゥンウォリアー","BO_WOODENWARRIOR"],
		[1168,5,"クリエイトウドゥンフェアリー","BO_WOODEN_FAIRY"],
		[1169,5,"クリエイトクリーパー","BO_CREEPER"],
		[1170,1,"リサーチレポート","BO_RESEARCHREPORT"],
		[1171,5,"クリエイトヘルツリー","BO_HELLTREE"],
		[1172,10,"ドラゴニックオーラ状態"],
		[1173,10,"天気修練","SKE_SKY_MASTERY"],
		[1174,10,"兵法修練","SKE_WAR_BOOK_MASTERY"],
		[1175,5,"天地一陽","SKE_RISING_SUN"],
		[1176,5,"太天一陽","SKE_NOON_BLAST"],
		[1177,5,"天陽","SKE_SUNSET_BLAST"],
		[1178,5,"天地一月","SKE_RISING_MOON"],
		[1179,5,"太天一月","SKE_MIDNIGHT_KICK"],
		[1180,5,"天月","SKE_DAWN_BREAK"],
		[1181,5,"天地万星","SKE_TWINKLING_GALAXY"],
		[1182,5,"天命落星","SKE_STAR_BURST"],
		[1183,5,"天星","SKE_STAR_CANNON"],
		[1184,10,"天羅万象","SKE_ALL_IN_THE_SKY"],
		[1185,10,"天気の身","SKE_ENCHANTING_SKY"],
		[1186,6,"運行の状態"],
		[1187,10,"護符修練","SOA_TALISMAN_MASTERY"],
		[1188,10,"霊道術修練","SOA_SOUL_MASTERY"],
		[1189,5,"(×)守護符","SOA_TALISMAN_OF_PROTECTION"],
		[1190,5,"武士符","SOA_TALISMAN_OF_WARRIOR"],
		[1191,5,"法師符","SOA_TALISMAN_OF_MAGICIAN"],
		[1192,5,"護魂一身","SOA_SOUL_GATHERING"],
		[1193,5,"(×)城隍堂","SOA_TOTEM_OF_TUTELARY"],
		[1194,5,"五行符","SOA_TALISMAN_OF_FIVE_ELEMENTS"],
		[1195,5,"霊道符","SOA_TALISMAN_OF_SOUL_STEALING"],
		[1196,5,"死霊浄化","SOA_EXORCISM_OF_MALICIOUS_SOUL"],
		[1197,5,"青龍符","SOA_TALISMAN_OF_BLUE_DRAGON"],
		[1198,5,"白虎符","SOA_TALISMAN_OF_WHITE_TIGER"],
		[1199,5,"朱雀符","SOA_TALISMAN_OF_RED_PHOENIX"],
		[1200,5,"玄武符","SOA_TALISMAN_OF_BLACK_TORTOISE"],
		[1201,5,"四方神符","SOA_TALISMAN_OF_FOUR_BEARING_GOD"],
		[1202,5,"四方五行陣","SOA_CIRCLE_OF_DIRECTIONS_AND_ELEMENTALS"],
		[1203,10,"天地神霊","SOA_SOUL_OF_HEAVEN_AND_EARTH"],
		[1204,5,"四方符状態"],
		[1205,10,"スピリットマスタリー","SH_MYSTICAL_CREATURE_MASTERY"],
		[1206,5,"三霊一体","SH_TEMPORARY_COMMUNION"],
		[1207,5,"にゃんブレッシング","SH_BLESSING_OF_MYSTICAL_CREATURES"],
		[1208,5,"マリンフェスティバル","SH_MARINE_FESTIVAL_OF_KI_SUL"],
		[1209,5,"サンドフェスティバル","SH_SANDY_FESTIVAL_OF_KI_SUL"],
		[1210,10,"独学 -戦闘学-","HN_SELFSTUDY_TATICS"],
		[1211,10,"独学 -魔導学-","HN_SELFSTUDY_SOCERY"],
		[1212,10,"P.F.I"],
		[1213,10,"グレネードマスタリー","NW_GRENADE_MASTERY"],
		[1214,1,"インテンシブエイム","NW_INTENSIVE_AIM"],
		[1215,10,"ヒドゥンカード","NW_HIDDEN_CARD"],
		[1216,5,"オンリーワンバレット","NW_ONLY_ONE_BULLET"],
		[1217,5,"スパイラルシューティング","NW_SPIRAL_SHOOTING"],
		[1218,5,"マガジンフォーワン","NW_MAGAZINE_FOR_ONE"],
		[1219,5,"ビジラントアットナイト","NW_THE_VIGILANTE_AT_NIGHT"],
		[1220,5,"ワイルドファイア","NW_WILD_FIRE"],
		[1221,7,"タイガースラッシュ","SH_CHUL_HO_SONIC_CLAW"],
		[1222,7,"タイガーハウリング","SH_HOWLING_OF_CHUL_HO"],
		[1223,7,"タイガーストライク","SH_HOGOGONG_STRIKE"],
		[1224,1,"にゃん友 -鉄虎-","SH_COMMUNE_WITH_CHUL_HO"],
		[1225,10,"影の舞","SS_KAGENOMAI"],
		[1226,10,"影一閃","SS_KAGEGISSEN"],
		[1227,10,"影狩り","SS_KAGEGARI"],
		[1228,1,"幻術 -影縫い-","SS_KAGEAKUMU"],
		[1229,10,"風魔手裏剣 -掌握-","SS_FUUMASHOUAKU"],
		[1230,10,"風魔手裏剣 -構築-","SS_FUUMAKOUCHIKU"],
		[1231,10,"ユピテルサンダーストーム","HN_JUPITEL_THUNDER_STORM"],
		[1232,10,"ヘルズドライブ","HN_HELLS_DRIVE"],
		[1233,10,"ナパームバルカンストライク","HN_NAPALM_VULCAN_STRIKE"],
		[1234,10,"メテオストームバスター","HN_METEOR_STORM_BUSTER"],
		[1235,10,"ダブルボウリングバッシュ","HN_DOUBLEBOWLINGBASH"],
		[1236,10,"メガソニックブロー","HN_MEGA_SONIC_BLOW"],
		[1237,10,"スパークブラスター","MT_SPARK_BLASTER"],
		[1238,5,"トリプルレーザー","MT_TRIPLE_LASER"],
		[1239,5,"エクスプロッシブパウダー","BO_EXPLOSIVE_POWDER"],
		[1240,10,"メイヘミックソーンズ","BO_MAYHEMIC_THORNS"],
		[1241,10,"ドラゴニックブレス","DK_DRAGONIC_BREATH"],
		[1242,10,"マイティスマッシュ","MT_MIGHTY_SMASH"],
		[1243,1,"アックスストンプ状態"],
		[1244,1,"(△)シールドシューティング状態"],
		[1245,1,"グランドジャッジメント状態"],
		[1246,1,"にゃん友 -賢鹿-","SH_COMMUNE_WITH_HYUN_ROK"],
		[1247,7,"ディアーキャノン","SH_HYUN_ROK_CANNON"],
		[1248,7,"ディアーブリーズ","SH_HYUN_ROKS_BREEZE"],
		[1249,5,"ベーシックグレネード","NW_BASIC_GRENADE"],
		[1250,5,"ヘイスティファイアインザホール","NW_HASTY_FIRE_IN_THE_HOLE"],
		[1251,5,"グレネーズドロッピング","NW_GRENADES_DROPPING"],
		[1252,10,"ミッションボンバード","NW_MISSION_BOMBARD"],
		[1253,10,"赤炎砲","SS_SEKIENHOU"],
		[1254,10,"冷血砲","SS_REIKETSUHOU"],
		[1255,10,"雷電砲","SS_RAIDENPOU"],
		[1256,10,"金龍砲","SS_KINRYUUHOU"],
		[1257,10,"暗転砲","SS_ANTENPOU"],
		[1258,1,"幻術 -暗黒龍-","SS_ANKOKURYUUAKUMU"],
		[1259,10,"暗転砲の習得Lv"],
		[1260,4,"太陽と月と星の日"],	//（太陽の日、月の日、星の日の判定用）
		[1261,5,"影潜り","SS_SHIMIRU"],
		[1262,5,"影溶き","SS_TOKEDASU"],
		[1263,10,"苦無 -歪曲-","SS_KUNAIWAIKYOKU"],
		[1264,5,"苦無 -回転-","SS_KUNAIKAITEN"],
		[1265,10,"苦無 -屈折-","SS_KUNAIKUSSETSU"],
		[1266,1,"幻術 -苦無-","SS_HITOUAKUMU"],
		[1267,10,"シールドチェーンラッシュ","HN_SHIELD_CHAIN_RUSH"],
		[1268,10,"スパイラルピアースマックス","HN_SPIRAL_PIERCE_MAX"],
		[1269,10,"ジャックフロストノヴァ","HN_JACK_FROST_NOVA"],
		[1270,10,"グラウンドグラビテーション","HN_GROUND_GRAVITATION"],
		[1271,1,"ブレイキングリミット状態"],
		[1272,1,"ルールブレイク状態"],
		[1273,1,"アビスダガー状態"],
		[1274,7,"レインボーホーン","SH_COLORS_OF_HYUN_ROK"],
		[1275,7,"(△)タートルスプリンクラー","SH_KI_SUL_WATER_SPRAYING"],
		[1276,7,"タートルランページ","SH_KI_SUL_RAMPAGE"],
		[1277,1,"にゃん友 -亀設-","SH_COMMUNE_WITH_KI_SUL"],
		[1278,1,"蜃気楼分身","SS_SHINKIROU"],
		[1279,1,"悪夢消し","SS_AKUMUKESU"],
		[1280,1,"ブレイキングリミット","HN_BREAKINGLIMIT"],
		[1281,1,"ルールブレイク","HN_RULEBREAK"],
		[1282,10,"流星落下 周辺追撃",""],
		[1283,5,"オートファイアリングランチャー","NW_AUTO_FIRING_LAUNCHER"],
		[1284,5,"ベーシックグレネード習得レベル",""],
		[1285,5,"ヘイスティファイアインザホール習得レベル",""],
		[1286,5,"グレネーズドロッピング習得レベル",""],
		[1287,7,"グレネードフラグメント","NW_GRENADE_FRAGMENT"],
		[1288,10,"ゴールデントーン","MH_GOLDENE_TONE"],
		[1289,10,"テンパリング","MH_TEMPERING"],
		[1290,5,"サイキックストリーム","EM_PSYCHIC_STREAM"],
		[1291,5,"ワイルドウォーク","WH_WILD_WALK"],
		[1292,5,"ラッシュストライク","MT_RUSH_STRIKE"],
		[1293,5,"パワフルスイング","MT_POWERFUL_SWING"],
		[1294,5,"エナジーキャノネード","MT_ENERGY_CANNONADE"],
		[1295,5,"ドラゴニックピアース","DK_DRAGONIC_PIERCE"],
		[1296,5,"ワイルドショット","NW_WILD_SHOT"],
		[1297,5,"ミッドナイトフォーリン","NW_MIDNIGHT_FALLEN"],
		[1298,5,"リズミカルウェーブ","TR_RHYTHMICAL_WAVE"],
		[1299,5,"天気身陽","SKE_SKY_SUN"],
		[1300,5,"天気身月","SKE_SKY_MOON"],
		[1301,5,"天星の行","SKE_STAR_LIGHT_KICK"],
		[1302,5,"烈火気弾","IQ_BLAZING_FLAME_BLAST"],
		[1303,7,"タイガーバトリング","SH_CHUL_HO_BATTERING"],
		[1304,7,"ディアースピリットパワー","SH_HYUN_ROK_SPIRIT_POWER"],
		[1305,5,"ヒットアンドスライディング","ABC_HIT_AND_SLIDING"],
		[1306,5,"チェイシングブレイク","ABC_CHASING_BREAK"],
		[1307,5,"チェイシングショット","ABC_CHASING_SHOT"],
		[1308,5,"アビスフレイム","ABC_ABYSS_FLAME"],
		[1309,10,"レイディアントスピア","IG_RADIANT_SPEAR"],
		[1310,5,"インペリアルクロス","IG_IMPERIAL_CROSS"],
		[1311,5,"インペリアルプレッシャー","IG_IMPERIAL_PRESSURE"],
		[1312,5,"クロススラッシュ","SHC_CROSS_SLASH"],
		[1313,5,"ディヴィヌスフロス","CD_DIVINUS_FLOS"],
		[1314,5,"ミステリーパウダー","BO_MYSTERY_POWDER"],
		[1315,5,"ダストエクスプロージョン","BO_DUST_EXPLOSION"],
		[1316,1,"四色符","SS_FOUR_CHARM"],
		[1317,5,"オーバーカミングクライシス","HN_OVERCOMING_CRISIS"],
		[1318,5,"エナジーコンバージョン","AG_ENERGY_CONVERSION"],
		// ドルイド
		[1319,1,"シェイプシフト：ウェアウルフ","DR_WEREWOLF"],
		[1320,1,"シェイプシフト：ウェアラプター","DR_WERERAPTOR"],
		[1321,10,"ビースティノーズ","DR_BEASTY_NOSE"],
		[1322,1,"トゥルースオブアイス","DR_TRUTH_OF_ICE"],
		[1323,1,"トゥルースオブウィンド","DR_TRUTH_OF_WIND"],
		[1324,1,"トゥルースオブアース","DR_TRUTH_OF_EARTH"],
		[1325,5,"ブラッドハウリング","DR_BLOOD_HOWLING"],
		[1326,5,"プリーニング","DR_PREENING"],
		[1327,10,"シャープアイズ","DR_SHARPE_EYES"],
		[1328,10,"(△)アイストーテム","DR_ICE_TOTEM"],
		[1329,10,"(△)カッティングウィンド","DR_CUTTING_WIND"],
		[1330,10,"(△)アースフラワー","DR_EARTH_FLOWER"],
		[1331,5,"エンレイジウルフ","DR_ENRAGE_WOLF"],
		[1332,5,"エンレイジラプター","DR_ENRAGE_RAPTOR"],
		[1333,10,"(△)アイスクラウド","DR_ICE_CLOUD"],
		[1334,10,"(△)ウィンドボム","DR_WIND_BOMB"],
		[1335,10,"(△)アラウンドフラワー","DR_AROUND_FLOWER"],
		[1336,10,"(△)ノーマーシークロー","DR_NOMERCY_CLAW"],
		[1337,10,"(△)シューティングフェザー","DR_SHOOTING_FEATHER"],
		[1338,10,"ネイチャーシールド","DR_NATURE_SHIELD"],
		[1339,10,"ネイチャーロジック","DR_NATURE_LOGIC"],
		[1340,10,"(△)クルーエルバイト","DR_CRUEL_BITE"],
		[1341,10,"(△)ローフライト","DR_LOW_FLIGHT"],
		[1342,10,"(△)ハンガー","DR_HUNGER"],
		[1343,10,"(△)フリッキングトルネード","DR_FLICKING_TONADO"],
		[1344,10,"(△)ダブルスラッシュ","KR_DOUBLE_SLASH"],
		[1345,10,"(△)シャープンガスト","KR_SHARPEN_GUST"],
		[1346,10,"ウルフインスティンクト","KR_WOLF_INSTINCT"],
		[1347,10,"(△)アイスピラー","KR_ICE_PILLAR"],
		[1348,10,"(△)サンダリングフォーカス","KR_THUNDERING_FOCUS"],
		[1349,5,"アースバド","KR_EARTH_BUD"],
		[1350,5,"(△)ナスティスラッシュ","KR_NASTY_SLASH"],
		[1351,10,"(△)シャープンヘイル","KR_SHARPEN_HAIL"],
		[1352,10,"ラプトリアルインスティンクト","KR_RAPTORIAL_INSTINCT"],
		[1353,10,"(△)アイススプラッシュ","KR_ICE_SPLASH"],
		[1354,10,"(△)サンダリングオーブ","KR_THUNDERING_ORB"],
		[1355,10,"(△)アースドリル","KR_EARTH_DRILL"],
		[1356,10,"(△)クローウェーブ","KR_CLAW_WAVE"],
		[1357,5,"(△)フェザースプリンクル","KR_FEATHER_SPRINKLE"],
		[1358,10,"(△)サンダリングコール","KR_THUNDERING_CALL"],
		[1359,10,"(△)アーススタンプ","KR_EARTH_STAMP"],
		[1360,10,"(△)チョップチョップ","KR_CHOP_CHOP"],
		[1361,10,"(△)タイフーンウィング","KR_TYPHOON_WING"],
		[1362,5,"ネイチャーヴィゴール","KR_NATURE_VIGOUR"],
		[1363,5,"ネイチャープロテクション","KR_NATURE_PROTECTION"],
		[1364,5,"アイアンハウリング","KR_IRON_HOWLING"],
		[1365,5,"ウィンドヴェール","KR_WIND_VEIL"],
		[1366,5,"パルスオブマッドネス","AT_PULSE_OF_MADNESS"],
		[1367,5,"フリップフラップ","AT_FLIP_FLAP"],
		[1368,5,"シックスセンス","AT_SIXTH_SENSE"],
		[1369,10,"(△)グレイシアモノリス","AT_GLACIER_MONOLITH"],
		[1370,5,"(△)ロアリングチャージ","AT_ROARING_CHARGE"],
		[1371,10,"(△)テラハーベスト","AT_TERRA_HARVEST"],
		[1372,5,"(△)プライマルクロー","AT_PRIMAL_CLAW"],
		[1373,10,"(△)ピニオンショット","AT_PINION_SHOT"],
		[1374,5,"アルファフェーズ","AT_ALPHA_PHASE"],
		[1375,10,"(△)グレイシアシャード","AT_GLACIER_SHARD"],
		[1376,10,"(△)ロアリングピアサー","AT_ROARING_PIERCER"],
		[1377,10,"(△)テラウェーブ","AT_TERRA_WAVE"],
		[1378,5,"(△)フェラルクロー","AT_FERAL_CLAW"],
		[1379,10,"(△)クイールスピア","AT_QUILL_SPEAR"],
		[1380,5,"エイペックスフェーズ","AT_APEX_PHASE"],
		[1381,5,"(△)グレイシアストンプ","AT_GLACIER_STOMP"],
		[1382,5,"(△)フューリアスストーム","AT_FURIOS_STORM"],
		[1383,5,"(△)ソリッドストンプ","AT_SOLID_STOMP"],
		[1384,5,"(△)アルファクロー","AT_ALPHA_CLAW"],
		[1385,10,"(△)テンペストフラップ","AT_TEMPEST_FLAP"],
		[1386,5,"(△)チリングブラスト","AT_CHILLING_BLAST"],
		[1387,5,"(△)フレンジファング","AT_FRENZY_FANG"],
		[1388,5,"エアロシンク","AT_AERO_SYNC"],
		[1389,5,"ネイチャーエイド","AT_NATURE_AID"],
		[1390,5,"ネイチャーハーモニー","AT_NATURE_HARMONY"],
		[1391,5,"(△)グラビティホール","AT_GRAVITY_HOLE"],
		[1392,10,"(△)サベージランジ","AT_SAVAGE_LUNGE"],
		[1393,1,"(△)グレイシアノヴァ","AT_GLACIER_NOVA"],
		[1394,1,"(△)グラウンドブルーム","AT_GROUND_BLOOM"],
		[1395,1,"ゼファーリンク","AT_ZEPHYR_LINK"],

	/*
	データ破損せず扱える SkillID の最大値は 11bit = 2048 です
	これを拡張したい場合は ro4/m/js/CSaveDataUnit.js の
	CSaveDataUnitAttackConf クラスの propNameAttackSkillID と
	CSaveDataUnitCharaConfSkill クラスの propNameSkillID の両方で
	該当プロパティの bit 数を更新した上で
	parse 関数をオーバーライドして後方互換性を確保してください
	オーバーライドされた parse の処理は CSaveDataUnitMob クラスを参考にしてください
	*/

	];

})();
